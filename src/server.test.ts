...
test("createServer registers exactly FIVE tools — 'delete_note' + 'help' + 'obsidian_exec' + 'read_note' + 'write_note' (FR-001 + FR-007 + P8 aggregator + BI-003 + BI-011 + BI-012)", async () => {
  const { ctx } = makeContext();
  const { server } = createServer(ctx);
  ...
  expect(names).toEqual(["delete_note", "help", "obsidian_exec", "read_note", "write_note"]);
});
