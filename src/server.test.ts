...
test("createServer registers exactly THIRTEEN tools — 'delete' + 'files' + 'find_by_property' + 'help' + 'obsidian_exec' + 'outline' + 'properties' + 'read' + 'read_heading' + 'read_property' + 'rename' + 'set_property' + 'write_note' (FR-001 + FR-007 + P8 aggregator + BI-003 + BI-011 + BI-012 + BI-013 + BI-014 + BI-015 + BI-018 + BI-019 + BI-021 + BI-022 + BI-023 + BI-024)", async () => {
  const { ctx } = makeContext();
  const { server } = createServer(ctx);
  ...
  expect(names).toEqual(["delete", "files", "find_by_property", "help", "obsidian_exec", "outline", "properties", "read", "read_heading", "read_property", "rename", "set_property", "write_note"]);
});
