...
test("createServer registers exactly SEVEN tools — 'delete_note' + 'find_by_property' + 'help' + 'obsidian_exec' + 'read_note' + 'read_property' + 'write_note' (FR-001 + FR-007 + P8 aggregator + BI-003 + BI-011 + BI-012 + BI-013 + BI-014)", async () => {
  const { ctx } = makeContext();
  const { server } = createServer(ctx);
  ...
  expect(names).toEqual(["delete_note", "find_by_property", "help", "obsidian_exec", "read_note", "read_property", "write_note"]);
});
