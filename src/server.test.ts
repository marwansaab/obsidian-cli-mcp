...
test("createServer registers exactly TWENTY tools — 'delete' + 'files' + 'find_by_property' + 'help' + 'links' + 'move' + 'obsidian_exec' + 'outline' + 'properties' + 'read' + 'read_heading' + 'read_property' + 'rename' + 'search' + 'set_property' + 'smart_connections_query' + 'smart_connections_similar' + 'tag' + 'paths' + 'write_note' (FR-001 + FR-007 + P8 aggregator + BI-003 + BI-011 + BI-012 + BI-013 + BI-014 + BI-015 + BI-018 + BI-019 + BI-021 + BI-022 + BI-023 + BI-024 + BI-025 + BI-026 + BI-027 + BI-028 + BI-029 + BI-030 + BI-033)", async () => {
  const { ctx } = makeContext();
  const { server } = createServer(ctx);
  ...
  expect(names).toEqual(["delete", "files", "find_by_property", "help", "links", "move", "obsidian_exec", "outline", "paths", "properties", "read", "read_heading", "read_property", "rename", "search", "set_property", "smart_connections_query", "smart_connections_similar", "tag", "write_note"]);
});
