// Original — no upstream. Tests for the files input schema — 18 cases per data-model.md test inventory covering target-mode interactions (specific-requires-vault, active-forbids-vault, file/path forbidden in both modes), folder/ext min(1) per R15, total boolean type, and strict-mode unknown-key rejection.
...
test("specific+vault+folder+ext happy (US1#1)", () => {
  const r = filesInputSchema.safeParse({
...
test("active no-vault happy", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active" });
  expect(r.success).toBe(true);
  if (r.success) {
...
test("specific without vault → issue on ['vault']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "specific" });
...
test("active with vault → issue on ['vault']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", vault: "V" });
  expect(r.success).toBe(false);
  if (!r.success) {
    expect(r.error.issues.some((i) => i.path.includes("vault"))).toBe(true);
  }
...
test("active with path → issue on ['path']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", path: "x.md" });
  expect(r.success).toBe(false);
  if (!r.success) {
    expect(r.error.issues.some((i) => i.path.includes("path"))).toBe(true);
  }
...
test("active with file → issue on ['file']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", file: "F" });
  expect(r.success).toBe(false);
  if (!r.success) {
    expect(r.error.issues.some((i) => i.path.includes("file"))).toBe(true);
  }
...
test("target_mode 'nope' → invalid_enum_value on ['target_mode']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "nope" });
  expect(r.success).toBe(false);
  if (!r.success) {
...
test("total 'true' (string) → invalid_type on ['total']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", total: "true" });
  expect(r.success).toBe(false);
  if (!r.success) {
...
test("total 1 (number) → invalid_type on ['total']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", total: 1 });
  expect(r.success).toBe(false);
  if (!r.success) {
    expect(r.error.issues.some((i) => i.path.includes("total"))).toBe(true);
  }
...
test("folder '' → too_small on ['folder'] (R15)", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", folder: "" });
  expect(r.success).toBe(false);
  if (!r.success) {
...
test("ext '' → too_small on ['ext'] (R15)", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", ext: "" });
  expect(r.success).toBe(false);
  if (!r.success) {
...
test("ext 5 (number) → invalid_type on ['ext']", () => {
  const r = filesInputSchema.safeParse({ target_mode: "active", ext: 5 });
  expect(r.success).toBe(false);
  if (!r.success) {
