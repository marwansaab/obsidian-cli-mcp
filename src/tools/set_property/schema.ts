// Original — no upstream. set_property input/output schemas — flat target-mode primitive extension; required name + value fields; four-shape value union (string | number | boolean | string[]); six-label write-side type enum (no "unknown" on write side); strict output { written: true, path, name }.
import { z } from "zod";
...
export const setPropertyInputSchema = applyTargetModeRefinement(
...
export type WritePropertyInput = z.infer<typeof setPropertyInputSchema>;
