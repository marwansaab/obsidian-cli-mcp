// Original — no upstream. Frozen JS template for the eval subcommand — base64 payload anti-injection (R6 / FR-020); walks `app.metadataCache.fileCache` × `app.metadataCache.metadataCache` directly (NOT a plugin API; sixth member of the eval-driven typed-tool cohort and the first core-metadataCache tag-index primitive); ASCII lower-fold inside the template (FR-008 / R14 amendment 1 — Obsidian's native `tag` subcommand is case-sensitive, contradicting Q1 defer-to-upstream); segment-bounded `isMatch` predicate (`tagLower === q || tagLower.startsWith(q + "/")`) enforces FR-016 child-tag subsumption + leaf-precision + substring-prefix rejection; per-path Set de-duplicates same-tag-multiple-occurrences (FR-007); `.md`-only filter (Obsidian tag cache indexes Markdown only); wrapper-side byte-asc `out.sort()` (R8 / FR-013 / Q5); envelope emit branched on `wantTotal` preserves cross-mode count invariant (FR-019). BI-034 (spec branch 034-fix-unicode-lookups): decode line uses the shared UTF-8-safe `B64_PAYLOAD_DECODE_EXPR`; the existing `String(payload.query).toLowerCase()` fold happens AFTER decode, so the fix is sufficient for non-ASCII tag queries.
import { B64_PAYLOAD_DECODE_EXPR } from "../_shared.js";

export const JS_TEMPLATE = `(()=>{
const payload=JSON.parse(${B64_PAYLOAD_DECODE_EXPR});
