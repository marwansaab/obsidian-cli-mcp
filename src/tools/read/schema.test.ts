...
test("rejects specific with neither file nor path (Story 4 AC#1)", () => {
  const result = readInputSchema.safeParse({ target_mode: "specific", vault: "MyVault" });
...
test("rejects specific with vault missing (Story 4 AC#4)", () => {
  const result = readInputSchema.safeParse({ target_mode: "specific", file: "F" });
  expect(result.success).toBe(false);
  if (!result.success) {
    const paths = result.error.issues.map((i) => i.path.join("."));
...
test("rejects forbidden 'vault' key in active mode (Story 3 AC#2)", () => {
  const result = readInputSchema.safeParse({ target_mode: "active", vault: "V" });
  expect(result.success).toBe(false);
  if (!result.success) {
...
test("rejects invalid discriminator value (Story 4 AC#5)", () => {
  const result = readInputSchema.safeParse({ target_mode: "unknown", vault: "V", file: "F" });
  expect(result.success).toBe(false);
  if (!result.success) {
