// Original — no upstream. Tests for the list_files tool registration — descriptor name + description, stripped JSON Schema (ADR-005), help-facility index reference, docs file presence + content completeness.
...
import { createListFilesTool, LIST_FILES_DESCRIPTION, LIST_FILES_TOOL_NAME } from "./index.js";
...
describe("createListFilesTool — descriptor", () => {
  // (1) descriptor name = "list_files"
  it("publishes name = 'list_files'", () => {
    const tool = createListFilesTool({ logger: silentLogger(), queue: createQueue(), spawnFn: makeStubSpawn() });
    expect(tool.descriptor.name).toBe(LIST_FILES_TOOL_NAME);
    expect(tool.descriptor.name).toBe("list_files");
  });

  // (2) description mentions target_mode, folder, total, and "help"
  it("description references target_mode, folder, total, list_files, and help", () => {
    const tool = createListFilesTool({ logger: silentLogger(), queue: createQueue(), spawnFn: makeStubSpawn() });
    expect(tool.descriptor.description).toBe(LIST_FILES_DESCRIPTION);
    const lower = LIST_FILES_DESCRIPTION.toLowerCase();
    expect(lower).toContain("target_mode");
    expect(lower).toContain("folder");
    expect(lower).toContain("total");
    expect(lower).toContain("list_files");
...
// (4) help facility references list_files — the help tool reads from the docs index
describe("help facility references list_files (registry-consistency)", () => {
  it("docs/tools/index.md mentions list_files", () => {
...
// (5) docs/tools/list_files.md exists and is non-stub
describe("docs/tools/list_files.md exists and is non-stub", () => {
  it("docs file resolves, no TODO marker, mentions inherited error codes, ≥4 example sections, has a known-limitations note", () => {
    const docsPath = resolve(dirname(fileURLToPath(import.meta.url)), "../../../docs/tools/list_files.md");
