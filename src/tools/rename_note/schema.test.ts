// Original — no upstream. Tests for the rename_note input + output schemas — happy paths across both modes, /speckit-clarify Q1 verbatim-`.md` forwarding accepted at schema layer, /speckit-clarify Q2 folder-separator rejection, Story 6 validation classes, UTF-8 byte-perfect, output literal-true gate.
import { expectTypeOf, test, expect } from "vitest";

import { renameNoteInputSchema, renameNoteOutputSchema, type RenameNoteInput } from "./schema.js";
...
test("specific+path+name happy path (Story 1 IT)", () => {
  const result = renameNoteInputSchema.safeParse({
...
test.each([
  ...
  const result = renameNoteInputSchema.safeParse(input);
  expect(result.success).toBe(false);
  if (!result.success) {
