...
- **`backlinks` typed MCP tool** at `src/tools/backlinks/` — `schema.ts` + `_template.ts` + `handler.ts` + `index.ts` + co-located tests (~60 cases: ~28 schema / ~30 handler / 5 registration). Single-call eval-cohort architecture (R1 / R2): one `invokeCli` per request routing through `obsidian eval code=<rendered-js>`; the frozen JS template reads `app.metadataCache.getBacklinksForFile(file)` via the `CustomArrayDict`'s `.keys()` / `.get(p)` method surface (T0 finding F3 — the dict's `.data` property is empty at this Obsidian release; deviation from data-model.md documented in research.md). Public surface: `backlinks({ target_mode, vault?, file?, path?, with_counts?, total?, limit? })` returning `{ count, backlinks: [{ source, count? }], truncated? }`. **`with_counts: true`** decorates each per-source entry with an integer `count` aggregating all references from that source (body wikilinks + body markdown links + body embeds + frontmatter wikilinks pre-merged uniformly per Obsidian's combined cache — verified by T0 F3 = 2). **`total: true`** returns the count alone with `backlinks: []` AND **BYPASSES the implicit 1000-source cap** per the 2026-05-17 Q1 clarification (FR-005a / SC-017). **`limit` (range 1..10000)** overrides the implicit cap; when the underlying source set exceeds the applied cap (entry-list modes only), the response carries `truncated: true` (R9). **Source corpus restricted to `.md` files** (case-insensitive extension match) per the 2026-05-17 Q2 clarification (FR-020a / R3); `.canvas` / `.base` / plugin-config / attachment sources excluded. **Self-references INCLUDED** (FR-013 — matches Obsidian's "Backlinks" pane semantic). **Aliased wikilinks attributed to the resolved target** (FR-015). **Frontmatter-declared references contribute uniformly with body references** (FR-016). **Code-block-only references excluded** by inheritance from Obsidian's link parser (FR-014 / verified by T0 F4). **Non-Markdown TARGET locators (`.canvas`, `.pdf`, attachments) rejected** via the eval's `f.extension === 'md'` guard mapped to `CLI_REPORTED_ERROR(stage:'envelope-error', code:'NOT_MARKDOWN')`. **Unknown vault display names** route through the inherited cli-adapter 011-R5 inspection clause to `CLI_REPORTED_ERROR(code:'VAULT_NOT_FOUND')` (FR-018 / verified by T0 F2). **Active-mode + no focused note** maps the eval-envelope `NO_ACTIVE_FILE` to `ERR_NO_ACTIVE_FILE` (BI-014 / BI-015 / BI-025 parity). **ZERO new top-level error codes**, **ZERO new `details.code` values** — the seventeen-tool zero-new-codes streak since BI-011 is preserved (Constitution Principle IV); the three envelope-error codes (`NO_ACTIVE_FILE`, `FILE_NOT_FOUND`, `NOT_MARKDOWN`) reuse the BI-025 set verbatim. **R12 anti-injection**: frozen JS template with single `__PAYLOAD_B64__` substitution; all user inputs flow through base64-encoded JSON; UTF-8-safe decode via shared `B64_PAYLOAD_DECODE_EXPR` (BI-034 inheritance).

- **Non-stub `docs/tools/backlinks.md`** per FR-026 — full input contract table for specific + active modes; output shapes for default / `with_counts` / `total` / truncated / self-reference variants; complete error roster (9 codes / stages); seven worked examples (specific-by-path / active-mode / with-counts / total-with-cap-bypass / capped-with-truncated / unresolved-path / non-Markdown-target); practical-ceiling notes (1000-source cap + 10 MiB output-cap + `total`-bypass); self-reference + frontmatter-inclusion + alias-attribution + code-block-exclusion + multi-vault + `.md`-only-corpus notes; cross-pointer to outgoing-links sibling `links`.

- **Two-line wiring** in `src/server.ts`: import + tools-array entry (alphabetical: inserted between `_register`-aggregated helpers and `createContextSearchTool` — `b` < `c`).

- **FR-026 baseline roll-forward**: `src/tools/_register-baseline.json` extended with the `backlinks` entry via `npm run baseline:write` (22 entries total). All other tool fingerprints unchanged byte-identically.

### Plan-stage / T0 findings

- **F1 — Native `obsidian backlinks` subcommand exists** with `path`/`file`/`counts`/`total`/`format=json` (probe at /speckit-implement T001). Documented but **NOT pivoted to** — the native subcommand lacks active-mode, returns a bare integer for `total` (not the locked envelope shape), emits `count` as a string, has no `limit` / `truncated` / `.md`-only-filter / `NOT_MARKDOWN`-guard guarantees, and uses field name `file` (not `source`). Pivoting would force a hybrid implementation (native for specific, eval for active) plus extensive wrapper-side transformation. The eval architecture gives one uniform code path. Recorded as a future-BI opportunity in [research.md § T0 Live-CLI Capture](specs/036-get-backlinks/research.md#t0-live-cli-capture-2026-05-17).
- **F3 — `CustomArrayDict.data` is empty; methods `.keys()` / `.get(p)` are load-bearing** (TRIGGER on the planned template). The data-model.md's planned access pattern (`Object.keys(dict.data)` + `data[p].length`) would have shipped silently broken — every call would return `{count: 0, backlinks: []}`. Template corrected in `src/tools/backlinks/_template.ts` to use the dict's documented method surface.
- **F2 + F4 confirmed** the eval-cohort R5 unknown-vault classification and the inherited R8 code-block exclusion respectively; no spec / plan amendments needed.

### Internal

- **Zero new ADR**, **zero Constitution amendment** — ADR-010 / ADR-013 / ADR-014 / ADR-015 all evaluated; only ADR-010 applies and the tool name `backlinks` is single-word verbatim from the upstream subcommand.
- **Four new source files** (`schema.ts` / `_template.ts` / `handler.ts` / `index.ts`), **three new test files** (`schema.test.ts` / `handler.test.ts` / `index.test.ts`); **two modified files** (`src/server.ts`, `src/tools/_register-baseline.json`); **two modified docs files** (`docs/tools/backlinks.md` NEW, `docs/tools/index.md` summary entry).
...
- **`smart_connections_query` typed MCP tool** at `src/tools/smart_connections_query/` — schema + handler + frozen `_template.ts` JS template + index + co-located tests (47 cases: 25 schema / 26 handler / 5 registration; minus a small overlap = 47 unique). Single-call architecture (R3) branched at envelope emission on `a.total`. NO `target_mode` discriminator (R4) — flat schema with optional `vault?`. Stage-1 LAST-`=> ` extraction (R14) because the plugin's `lookup` API emits plugin-side `console.log("Found and returned N smart_blocks.")` AND `[warn]` lines on stdout BEFORE the `=> ` eval-return marker. In-eval lookup-error-sentinel detection (R11) via `if (r && r.error)` check — the plugin returns `{error: <string>}` sentinels (not throws) for empty hypotheticals and for missing/misconfigured embed model. In-eval API-shape check (R12) via `typeof env.smart_sources?.lookup !== 'function'`. Per-match transform from top-level `m.key` + `m.score` to `{path, headingPath, score}` (the plugin's `m.item` field carries CIRCULAR back-references and is NEVER serialised — F7 critical finding). Three-level sort intra-eval (R8). NO self-exclusion (R9 N/A — no source note). `Number.isFinite` score filter (R10). Public surface: `smart_connections_query({ query, vault?, limit?, total? })` → `{ count, matches: [{ path, headingPath, score }] }`. Schema cap: `query: z.string().trim().min(1).max(4000)`. FIVE-entry failure-mode roster: `VALIDATION_ERROR`, `CLI_REPORTED_ERROR(VAULT_NOT_FOUND, reason: "unknown" | "not-open")`, `CLI_REPORTED_ERROR(SMART_CONNECTIONS_NOT_INSTALLED)`, `CLI_REPORTED_ERROR(SMART_CONNECTIONS_NOT_READY, reason: "api-missing" | "embed-failed")`. NEW emission for BI-027: the `embed-failed` sub-reason (detected via the lookup return-value sentinel check); the `api-missing` sub-reason (detected via the in-eval API-shape check). Zero new top-level error codes — the thirteen-tool zero-new-top-level-codes streak since BI-011 is preserved (Constitution Principle IV).
- **NEW cross-cutting shared module** `src/tools/_eval-vault-closed-detection/{detector, registry-parser, index}.ts` extracted in this BI per FR-020 / Q8(c) hybrid extraction. Plugin-AGNOSTIC — characterises the Obsidian CLI's empty-stdout + transparent-open response to `vault=<closed-eval>`; any future eval-driven typed tool with a `vault?` parameter MAY consume it. 20 co-located tests (12 detector / 8 parser). The cli-adapter remains FROZEN (008-refactor surface invariant) — the shared module sits ONE LAYER UP.
- **Non-stub `docs/tools/smart_connections_query.md`** per FR-021 / FR-022 — input table, output shape × 2 modes (default + count-only), four worked examples (default focused-vault / explicit-vault count-only / small-limit + frontmatter-block / closed-but-registered-vault retry pattern), full 5-entry error roster including the two `details.reason` sub-discriminators, error-precedence chain diagram per FR-017, eight documented inherited limitations (embedding-model-dependent score bands; indexing freshness; folder exclusions; plugin-version drift; local-model silent query truncation; embed-call latency cap (10s); stale-index reverse direction; low-information query fallback).
- **One-line entry** in `docs/tools/index.md` between `set_property` and `smart_connections_similar`.
- **Two-line wiring** in `src/server.ts`: import + tools-array entry (alphabetical: inserted between `createSetPropertyTool` and `createSmartConnectionsSimilarTool`).
- **FR-018 baseline roll-forward**: `src/tools/_register-baseline.json` extended with the `smart_connections_query` entry via `npm run baseline:write`. The existing `smart_connections_similar` fingerprint is unchanged across the BI-026 ripple refactor (cohort-consistency gate).

### Migration

None. Additive surface — existing tools unchanged at the wire. The BI-026 ripple to `smart_connections_similar` is additive at the wire (existing pattern-matchers on `details.code` still match; the new `details.reason: "api-missing"` field on `SMART_CONNECTIONS_NOT_READY` is supplementary).

### Internal

- **BI-026 ripples** (FR-013a + FR-020a): (a) refactored `src/tools/smart_connections_similar/handler.ts` to consume the shared `_eval-vault-closed-detection` detector — behaviour-preserving; the inline `isVaultRegistered` helper was removed; (b) added `details.reason: "api-missing"` to `smart_connections_similar`'s existing `SMART_CONNECTIONS_NOT_READY` emission for cohort-wide exhaustiveness of the `details.reason` field per ADR-015 worked-example pattern. Three new regression tests at `src/tools/smart_connections_similar/handler.test.ts` lock the cohort consistency.
- **Frozen surfaces**: `obsidian_exec`, `read`, `write_note`, `delete`, `read_property`, `find_by_property`, `read_heading`, `set_property`, `files`, `rename`, `outline`, `properties`, `links`, `help` all byte-stable.
- **Plan-stage live-probe-driven amendments** (per FR-024 protocol, 2026-05-15): amendment 1 — `hypotheticals` lives at the TOP LEVEL of `lookup({...})` params, NOT inside `filter` (verified via plugin source inspection); amendment 2 — lookup errors return as `{error: <string>}` SENTINELS, not thrown exceptions, and plugin-side `console.log` output captures to stdout BEFORE the `=> ` eval-return marker (drives the LAST-`=> ` extraction strategy in stage-1).
- **/speckit-clarify Q&As 2026-05-15**: Q1 embed-call timeout → defer to cli-adapter 10s cap (new inherited limitation #6); Q2 stale-index reverse direction (match references deleted file) → pass through unchanged (new inherited limitation #7).
- Zero new ADRs (ADR-013 / ADR-014 / ADR-015 cover this BI as their second consumer). Zero Constitution amendment.

### References

- Spec: [specs/027-smart-connections-query/spec.md](specs/027-smart-connections-query/spec.md)
- Plan: [specs/027-smart-connections-query/plan.md](specs/027-smart-connections-query/plan.md)
- Research: [specs/027-smart-connections-query/research.md](specs/027-smart-connections-query/research.md)
- Data model: [specs/027-smart-connections-query/data-model.md](specs/027-smart-connections-query/data-model.md)
- Tasks: [specs/027-smart-connections-query/tasks.md](specs/027-smart-connections-query/tasks.md)
- Quickstart: [specs/027-smart-connections-query/quickstart.md](specs/027-smart-connections-query/quickstart.md)
- Input contract: [specs/027-smart-connections-query/contracts/smart-connections-query-input.contract.md](specs/027-smart-connections-query/contracts/smart-connections-query-input.contract.md)
- Handler contract: [specs/027-smart-connections-query/contracts/smart-connections-query-handler.contract.md](specs/027-smart-connections-query/contracts/smart-connections-query-handler.contract.md)
- ADR-013: [.decisions/ADR-013 - Plugin-Namespace Tool Naming Convention.md](.decisions/ADR-013%20-%20Plugin-Namespace%20Tool%20Naming%20Convention.md)
- ADR-014: [.decisions/ADR-014 - Plugin-Backed Typed Tools Runtime-Dependency Pattern.md](.decisions/ADR-014%20-%20Plugin-Backed%20Typed%20Tools%20Runtime-Dependency%20Pattern.md)
- ADR-015: [.decisions/ADR-015 - Sub-Discriminators via details.reason for Multi-State Error Codes.md](.decisions/ADR-015%20-%20Sub-Discriminators%20via%20details.reason%20for%20Multi-State%20Error%20Codes.md)

## [0.5.4] - 2026-05-15

**PATCH release (additive surface)** — adds `smart_connections_similar`, the thirteenth typed-tool wrap and the project's **first plugin-backed typed surface**. Returns a typed list of semantically-similar block-level matches for a single source note via the Smart Connections plugin's similarity API as a `{ count, matches: [{ path, headingPath, score }] }` envelope. Wraps the upstream Obsidian CLI's `eval` subcommand under the hood (NOT a native subcommand — `obsidian help` confirms zero similarity-related subcommands; the wrapper routes through `app.plugins.plugins["smart-connections"].env.smart_sources.items[<key>].find_connections({limit})`). The `total: true` switch returns the count alone with `matches: []` for a token-economical pre-flight read. Replaces "obsidian_exec into the plugin's chat surface plus client-side parse" with a single typed call. Spec-id 026-smart-connections-similar, FR-001..FR-030a, SC-001..SC-028a.

### Added

- **`smart_connections_similar` typed MCP tool** at `src/tools/smart_connections_similar/` — schema + handler + frozen `_template.ts` JS template + index + co-located tests (76+ cases: 32 schema / 38 handler / 5 registration; exceeds SC-021 floor of 50). Single-spawn architecture (R3): ONE `invokeCli` call per request with `subcommand: eval` and `parameters.code: <rendered-js>`; the `a.total` branch lives INSIDE the eval at envelope emission, so the cross-mode invariant (FR-006a — `count_default === count_total_only`) holds by construction. Per-match in-eval transforms: (a) split-on-first-`#` for `path`/`headingPath` per R7/F4/F6 (frontmatter sentinel `"---frontmatter---"` preserved verbatim); (b) `Number.isFinite(score)` filter per R10/Q2 silently drops non-finite scores; (c) source-path-keyed self-exclusion per R9/FR-010 removes source AND block-inside-source matches; (d) three-level sort score-desc / path-byte-asc / headingPath.join('#')-byte-asc per R8/FR-008. Public surface: `smart_connections_similar({ target_mode, vault?, file?, path?, limit?, total? })` → `{ count, matches: [{ path, headingPath, score }] }`. STANDARD target-mode discriminator (parity with `read` / `read_heading` / `links`). NEW handler-side **stage-0 closed-but-registered-vault detection branch** (R5a / FR-017a) — on empty stdout + exit 0 + specific mode + vault registered (verified by a second `obsidian vaults verbose` invokeCli), surfaces `CLI_REPORTED_ERROR(details.code: "VAULT_NOT_FOUND", details.reason: "not-open")`; the cli-adapter's 008-refactor surface stays FROZEN. NEW eight-entry envelope error roster (NO_ACTIVE_FILE / FILE_NOT_FOUND / NOT_MARKDOWN / SMART_CONNECTIONS_NOT_INSTALLED / SMART_CONNECTIONS_NOT_READY / SOURCE_NOT_INDEXED, plus the two VAULT_NOT_FOUND sub-discriminator reasons) — ALL via `CLI_REPORTED_ERROR` with `details.code` discriminator; **zero new top-level error codes** (FR-021); the eleven-tool zero-new-top-level-codes streak since BI-011 is preserved.
- **Non-stub `docs/tools/smart_connections_similar.md`** per FR-022 — input contract per mode, output shape × 2 modes, block-vs-source granularity note with client-side collapse recipe, six worked examples (indexed-note by path / by basename / active-mode / count-only pre-flight / plugin-not-installed / closed-but-registered-vault retry pattern), full error roster covering all 13 surfaces incl. the two `details.reason` sub-discriminators, error-precedence chain diagram per FR-017b, plugin-as-runtime-dependency note with the minimum probed plugin version (Smart Connections v4.5.0 by Brian Petro) soft-pinned per Q1, five documented inherited limitations (embedding-model-dependent score bands; indexing freshness; folder exclusions; plugin-version drift surfaces as NOT_READY; multi-vault basename ambiguity), out-of-scope upstream surfaces section, practical-ceiling note.
- **One-line entry** in `docs/tools/index.md` between `set_property` and `write_note`.
- **Two-line wiring** in `src/server.ts`: import + tools-array entry (alphabetical: inserted between `createSetPropertyTool` and `createWriteNoteTool`).
- **FR-018 baseline roll-forward**: `src/tools/_register-baseline.json` extended with the `smart_connections_similar` entry via `npm run baseline:write`. All other tool fingerprints unchanged byte-identically (SC-019).
- **`_register.test.ts` invariants map** extended with the `smart_connections_similar` entry (`properties_equals_set: ["target_mode", "vault", "file", "path", "limit", "total"]`, `required_equals: ["target_mode"]`, `additionalProperties: false`).

### New ADR / Constitution amendment

- **ADR-013 — Plugin-Namespace Tool Naming Convention** created at `.decisions/ADR-013 - Plugin-Namespace Tool Naming Convention.md`. Codifies the `<plugin_name>_<operation>` naming pattern (e.g. `smart_connections_similar`). Sibling rule to ADR-010 (single-word-verbatim-from-upstream for native CLI subcommand wrappers); mutually exclusive in scope and exhaustive over the typed-tool naming space.
- **Constitution v1.3.0 → v1.4.0 amendment** at `.specify/memory/constitution.md` adds a seventh Compliance checklist row pointing at ADR-013. No principle text changes.

### Plan-stage design decisions

- **R2 — `eval` subcommand load-bearing (NO native similarity subcommand)**: probed live 2026-05-15 (F2). `obsidian help` confirms zero similarity-related subcommands. The wrapper MUST route through `eval` to reach the plugin's similarity API. First member of a new **eval-driven plugin-backed cohort** routing into a plugin's runtime object rather than Obsidian's core APIs (parity with BI-014 / BI-015 / BI-025 eval cohort otherwise).
- **R5a — Closed-but-registered vault detection branch (NEW for BI-026)**: probed live 2026-05-15 (F7 / F8 / F9). The CLI emits empty stdout + exit 0 for the FIRST `eval` call against a closed registered vault AND transparently OPENS the vault as a side effect. The cli-adapter's 011-R5 inspection clause does NOT fire (no "Vault not found." string). Detection lives in the typed-tool handler via a stage-0 branch consulting `obsidian vaults verbose` to distinguish "registered but not open" from a genuinely-anomalous empty-stdout response. The 008-refactor surface stays FROZEN.
- **Live-probe-driven amendments** (post-test 2026-05-15): amendment 1 contradicted the spec-stage Q3 premise that `vault=` routing would surface a "wrong-vault" failure — live probe confirmed `vault=` routes correctly to the named vault's `app` instance; FR-017a repurposed for closed-but-registered-vault detection. Amendment 2 contradicted the grilling-Q3 source-only granularity assumption — live probe confirmed `find_connections()` returns block-level matches by default; v1 per-match shape switched from `{path, score}` to `{path, headingPath, score}` with the frontmatter sentinel `"---frontmatter---"` preserved verbatim.
- **T0.1 — active-mode NO_ACTIVE_FILE envelope code aligns with BI-015 / BI-025 precedent**: maps to `ERR_NO_ACTIVE_FILE` (not `CLI_REPORTED_ERROR`).
- **Q1–Q5 clarifications session 2026-05-15** (codified in spec.md): Q1 docs-only soft-pin for minimum plugin version; Q2 silently drop non-finite scores; Q3 vault-mismatch detection (REVISED by live-probe amendment to `not-open` closed-vault detection); Q4 outer-to-inner / cheapest-first error precedence chain (FR-017b); Q5 architecture-doc snapshot semantics (FR-030 / FR-030a).

### Internal

- **Frozen surfaces** (SC-019): `obsidian_exec`, `read`, `write_note`, `delete`, `read_property`, `find_by_property`, `read_heading`, `set_property`, `files`, `rename`, `outline`, `properties`, `links`, `help` all byte-stable — input schemas, output shapes, and error codes unchanged. Zero new top-level error codes (FR-021, Constitution IV); one new ADR (ADR-013); one constitution version bump (v1.3.0 → v1.4.0).
- **Drift detector**: the post-010 consolidated drift detector's `it.each` registry walk auto-covers `smart_connections_similar`; the invariants map row added.

### References

- Spec: [specs/026-smart-connections-similar/spec.md](specs/026-smart-connections-similar/spec.md)
- Plan: [specs/026-smart-connections-similar/plan.md](specs/026-smart-connections-similar/plan.md)
- Research: [specs/026-smart-connections-similar/research.md](specs/026-smart-connections-similar/research.md)
- Data model: [specs/026-smart-connections-similar/data-model.md](specs/026-smart-connections-similar/data-model.md)
- Tasks: [specs/026-smart-connections-similar/tasks.md](specs/026-smart-connections-similar/tasks.md)
- Quickstart: [specs/026-smart-connections-similar/quickstart.md](specs/026-smart-connections-similar/quickstart.md)
- Input contract: [specs/026-smart-connections-similar/contracts/smart-connections-similar-input.contract.md](specs/026-smart-connections-similar/contracts/smart-connections-similar-input.contract.md)
- Handler contract: [specs/026-smart-connections-similar/contracts/smart-connections-similar-handler.contract.md](specs/026-smart-connections-similar/contracts/smart-connections-similar-handler.contract.md)
- ADR-013: [.decisions/ADR-013 - Plugin-Namespace Tool Naming Convention.md](.decisions/ADR-013%20-%20Plugin-Namespace%20Tool%20Naming%20Convention.md)
