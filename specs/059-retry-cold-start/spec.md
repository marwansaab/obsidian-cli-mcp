...
- Q: Should the intermittent transport-level `Stream closed` first-attempt failure also trigger the single retry, or only the exact `exitCode: 0` + "command not found" stdout signature? → A: Yes — a first-attempt `Stream closed` triggers the same single retry, treated as an alternate manifestation of the cold-launch window. It is bounded and self-limiting (one retry, then the second attempt is authoritative), so retrying a `Stream closed` that turns out to be a genuine transport failure is harmless. A `Stream closed` returned by the retry itself just propagates per FR-007.
...
- **First attempt surfaces the transport-level variant instead of the not-found signature**: ADR-029 notes the first attempt occasionally surfaces as a transport-level `Stream closed` rather than the `exitCode: 0` + "command not found" stdout. This variant triggers the same single retry (FR-001, Clarification 2026-05-30); the second attempt is then authoritative. Its empirical frequency (reliable vs intermittent) is still characterised by the plan-phase T0 probe (OQ-002), but the inclusion decision is settled.
...
- **Cold-start failure signature**: The observable signal that identifies the transient cold-launch window, in two recognized first-attempt forms (Clarification 2026-05-30): (a) the canonical `exitCode: 0` together with the stdout `Error: Command "<cmd>" not found. It may require a plugin to be enabled.` (ADR-029); and (b) the intermittent transport-level `Stream closed` failure. Either form is the trigger for the retry. Form (a) is observably indistinguishable from a genuinely-unknown-command failure, which is why the retry must be self-limiting rather than signature-discriminating.
...
- **OQ-002 — Transport-level variant** (inclusion decided 2026-05-30: it triggers the retry — FR-001): the remaining probe is to characterise how reliably vs intermittently the `Stream closed` first-attempt failure occurs during cold-start, to confirm the trigger match and inform the test fixtures. No longer an open decision, only an empirical characterisation.
