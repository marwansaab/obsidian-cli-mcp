...
- **Form (a)** — `value instanceof UpstreamError && value.code === "CLI_REPORTED_ERROR" && typeof value.details?.stdout === "string" && value.details.stdout.includes(COLD_START_INVARIANT)` where `COLD_START_INVARIANT = "not found. It may require a plugin to be enabled."` (a single named constant; substring, command-name-independent, per Q4).
