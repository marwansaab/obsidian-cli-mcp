...
- [ ] T009 [P] [US1] Write 6 stdout-parsing + sort tests in [src/tools/list_files/handler.test.ts](../../src/tools/list_files/handler.test.ts) per cases #6–#11: 3-path stdout → sorted response (case #6); unsorted stdout → lex-sorted response (case #7); non-BMP character stdout (e.g. emoji-named file) → UTF-8 byte-order (DIFFERS from JavaScript default for non-BMP; case #8); empty stdout → `{ count: 0, paths: [] }` — also gates SC-021 folder-names-a-file conflation (case #9); trailing-newline-only stdout → empty response (case #10); mixed empty lines → empties dropped (case #11).
