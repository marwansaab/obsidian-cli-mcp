...
4. **Given** a note with `tags: [alpha, beta]`, **When** the agent calls with `arrayMatch: false` and `value: ["beta", "alpha"]` (same elements, different order), **Then** the observable behaviour MUST be characterised in the live-CLI characterisation pass and documented; the spec does NOT pre-commit order-sensitivity (it is a Phase 0 research output).
...
- The `folder` field is a vault-relative path prefix. A caller-supplied `folder` of `..` (or `../escape`) MUST either be rejected at the validation boundary OR MUST verifiably yield zero matches because the underlying CLI's folder-scoping operates against the vault root and does not honour `..` escapes. The implementation MUST close the loop on this one of two ways and the choice MUST be documented; a `folder: ".."` query that returns content from outside the vault root is a security defect that MUST NOT ship.
...
- **FR-003**: The tool MUST accept an optional `vault` field (a string, the vault display name). When omitted, the underlying CLI's focused-vault default is used.
...
- **FR-021**: The `folder` field MUST be either rejected at the validation boundary when it contains a path-traversal escape (e.g. starts with `..` or `/`), OR MUST verifiably yield zero matches because the underlying CLI's folder-scoping does not honour escapes. The choice MUST be documented and the chosen mechanism MUST be locked by at least one regression test.
...
- **FR-025**: The tool MUST be registered through the project's existing typed-tool registration factory. The progressive-disclosure help facility's documentation file for `find_by_property` MUST be authored with the per-field input contract, the output shape, the failure-mode roster, and at least four worked examples covering scalar happy-path, folder-scoped, array-contains, and case-insensitive.
...
- **SC-010**: A `folder` value that contains a path-traversal escape (`..`, absolute `/` prefix) is either rejected at the validation boundary OR verifiably yields zero matches; in 100% of test runs no path outside the vault root appears in the response.
...
- The user input is exhaustive for ship-gating decisions. The 16 acceptance criteria across [P1] / [P2] / [P3], the six adversarial categories (CONCURRENCY, CONTENT, LIMITS, UNDERLYING, CLIENT-CLASS, SECURITY), and the explicit out-of-scope list cover the spec surface; no clarifications session is anticipated. If planning surfaces an undecidable contract (most likely the array-exact-equality order sensitivity per FR-016 and FR-027), `/speckit-clarify` will run before plan-stage finalisation.
