...
## D5 — US1: label-strip, punctuation-safe — **TAB delimiter confirmed (T0 P1)**

- **Decision**: Replace the current pass-through line-split (which trims whitespace only) with a precise strip of the injected type label, anchored to the **known Bases view-type token set**. The strip removes only the trailing label (and its delimiter); it never blind-trims a trailing token, so legitimate internal/trailing spaces and punctuation survive (FR-003 / SC-003).
- **T0 P1 evidence** (see [contracts/t0-probe-findings.md](contracts/t0-probe-findings.md)): the real `base:views` emission is `<view name>\t<type>` — the delimiter is a **TAB**, not a space, and the closed view-type token set captured live from the Bases internal-plugin registry (`app.internalPlugins.plugins.bases.instance.registrations`) is **`["table", "cards", "list"]`** (Obsidian 1.12.7). Because a TAB cannot appear inside a view name, the name/type split is unambiguous and internal spaces/hyphens/punctuation are preserved for free; the known-type anchoring is retained as a defence-in-depth guard (`stripTypeLabel` strips the trailing tab-delimited token only when it ∈ {`table`, `cards`, `list`}). This corrects D5's original *space*-delimiter assumption.
- **Rationale**: The returned name must equal the name `query_base` accepts (FR-002 / SC-001). The current handler returns `"<name>\t<type>"` verbatim. Anchoring to known type tokens + the tab delimiter satisfies "preserve internal punctuation" without a separate clean-name channel from upstream (which `base:views` does not provide).
- **Alternatives**: Split on the last delimiter unconditionally — superseded by evidence: with a TAB delimiter this is now safe, but the known-type guard is kept per spec intent. Ask upstream for structured output — rejected; `base:views` has no JSON/structured mode (T0 P1 confirms the TSV-style format). Parse the `.base` YAML for names — rejected as primary (BI-041 norm; new-ADR territory).
...
**Output**: all decisions recorded; no `NEEDS CLARIFICATION` remains. The mechanism arm was selected at implement-time by the T0 forcing-gate probe: **resolved arm = native focus-first** (T007). P2 confirmed `base:views` ignores `path=`/`vault=` (active-only, R-003 holds), eliminating the single-call `path=` arm; P3 confirmed focus-then-active is reliable (no race), eliminating the eval fallback. The label delimiter is a **TAB** with the closed type set `{table, cards, list}`. Full evidence in [contracts/t0-probe-findings.md](contracts/t0-probe-findings.md).
