...
- **Errors.** Reuse `CLI_REPORTED_ERROR` sub-discriminators + `VALIDATION_ERROR`; **zero new top-level codes**. Named-not-found surfaces distinctly from no-base-open (the focus arm yields the open mechanism's `FILE_NOT_FOUND`; a new `details.reason` on `BASE_NOT_FOUND` is introduced **only if** the resolved arm cannot otherwise distinguish them — ADR-015 additive-only, "only if needed").
...
**Constraints**: Zero new top-level error code (Principle IV); `details.reason` only if needed for named-vs-open distinguishability (ADR-015 additive); names-only output (shape `{views, count}` unchanged); read-only (FR-011 — focusing a Base changes which file is active, an accepted side effect of the named path, documented; the open Base path mutates nothing); label-strip must preserve legitimate punctuation (FR-003); no tool→tool upward import (Principle I — the focus step composes shared eval primitives, it does not import the `open_file` module).
**Scale/Scope**: One existing module `src/tools/views_base/**` (`schema.ts` + `handler.ts` + `index.ts` + their three co-located `*.test.ts`), one `docs/tools/views_base.md` rewrite, one `_register-baseline.json` fingerprint regen for `views_base`. **No edits to** `server.ts` (already registers `views_base`), `cli-adapter.ts`, `_dispatch.ts`, `errors.ts`, `logger.ts`, `queue.ts`, or any sibling tool module.
...
| **ADR-015** (sub-discriminators via `details.reason`) | **Y (conditional)** | A new `BASE_NOT_FOUND` `details.reason` (`named-missing` vs `not-open`) is introduced **only if** the resolved arm cannot otherwise distinguish named-not-found from no-base-open (the focus arm yields distinct `FILE_NOT_FOUND`, needing none). Additive-only; zero new top-level codes either way. Settled by the T0 probe outcome and recorded in data-model. |

**No `N` verdicts → no Complexity Tracking entry required.**

**Kernel-node attention (per CLAUDE.md)**: touches **none** of the four kernel nodes' definitions. `createLogger` / `createQueue` are injected via `ExecuteDeps` (the handler never constructs/imports them — DI discipline preserved); `UpstreamError` is used as a value type, not modified; `createServer` is **not** touched (`views_base` is already registered — this BI modifies an existing tool, it adds no registration line). Blast radius: one existing leaf module in the native Bases family + its baseline fingerprint + its doc.
...
- **D8 — Error roster, zero new top-level codes.** named-not-found (focus `FILE_NOT_FOUND`) vs no-base-open (`BASE_NOT_FOUND`) vs invalid-locator (`VALIDATION_ERROR`/`INVALID_BASE_PATH`) vs malformed (`BASE_MALFORMED`) vs bad-vault (`VAULT_NOT_FOUND`). `details.reason` added only if an arm collapses named-not-found into `BASE_NOT_FOUND` (ADR-015, "only if needed"). No silent open-Base substitution (FR-009).
...
Re-evaluated after data-model/contracts: input is the shared zod `superRefine` (single source of truth, `z.infer` downstream; III); output unchanged strict object (III); zero new top-level codes, `details.reason` additive-and-conditional, no silent open-Base substitution (IV); change confined to the native Bases-family module with no sibling import and no kernel-node touch (I); every modified surface keeps co-located happy + failure tests (II); `// Original — no upstream.` headers retained (V); ADR-010 unchanged, ADR-013/014 N/A, ADR-015 additive-only. **No gate regressed; no violations; Complexity Tracking empty.**
...
**Kernel-node touch surface**: `createLogger` / `createQueue` / `UpstreamError` / `createServer` — **none modified**. `createLogger`/`createQueue` injected (not imported by the handler); `UpstreamError` used as a value; `createServer` untouched (`views_base` already registered). Verifies the no-touch claim the post-implement step checks.
