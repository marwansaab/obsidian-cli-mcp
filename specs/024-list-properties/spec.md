...
- **SC-012**: Count-only mode returns `{ count: N, properties: [] }` for both N > 0 and N = 0 cases in 100% of test runs; count-only mode also surfaces the unknown-vault error path when the `vault` parameter is unknown (does NOT short-circuit it). The cross-mode invariant (FR-006a) is verified: the same vault state returns the same outer `count` value whether queried with `total: false` or `total: true` in 100% of test runs.
