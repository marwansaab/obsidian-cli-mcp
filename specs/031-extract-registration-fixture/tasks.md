...
**Goal**: Demonstrate that a future fix to the shared fixture propagates to all 16 consumers automatically.
...
US2 has no implementation tasks of its own. The standing-wave benefit is automatically satisfied once Phase 3 ships — the consolidation IS the deliverable; any future fix to `_registration-stub.ts` propagates by the nature of TypeScript module imports. The co-located test from T003 ensures that future fixes have a verification target. The single task below is an OPTIONAL acceptance check the implementer MAY run to confirm the propagation property; it is not gated on US1 completion beyond Phase 3's verification.
