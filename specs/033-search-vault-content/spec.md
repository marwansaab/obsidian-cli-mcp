...
- **SC-009**: Invalid inputs (empty query, query longer than 1000 characters, `limit` outside `1..10000`, unknown parameter, `mode: 'active'` + `vault` name) are rejected before any vault scan with a structured `VALIDATION_ERROR`.
