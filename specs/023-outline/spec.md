...
- F: Live-CLI probe of `obsidian outline format=json` against a fixture containing both ATX and Setext-underline-style headings showed that **the upstream CLI INCLUDES Setext headings** in its outline output (the Setext text line surfaces as a heading entry with the depth implied by the underline character — `====` for level 1, `----` for level 2). This contradicts the spec-stage assumption that drove FR-013 (which was based on the BI-015 precedent). → Plan applies the same defer-to-upstream architectural pattern locked for indented-code-blocks in clarifications-session Q2/A2: the wrapper does NOT filter Setext entries; the outline reflects whatever upstream emits. Locks an amendment to FR-013 (rewritten from "MUST NOT be returned" to "defers to upstream"); the CONTENT — Setext edge case is rewritten in parallel; the FR-023 characterisation case is reworded from "verifies they are NOT included" to "verifies the deferred-to-upstream contract"; out-of-scope assumption updated to drop the Setext fallback documentation (Setext IS now in reach via this tool, just not via heading-path-targeted reads in `read_heading`). The alternative (wrapper-side Setext filter) would require a second invocation to read file content for the `#`-prefix check on each heading line, defeating the single-call-per-request architecture. See [research.md F10 / R11](./research.md).
...
**CONTENT — Setext underline-style headings (deferred-to-upstream)**

- Headings written in Setext underline form (a text line followed by `====` for H1 or `----` for H2) ARE included in the outline by the upstream CLI (verified 2026-05-13 via plan-stage live probe). Per the deferred-to-upstream architectural pattern locked in the 2026-05-13 clarifications session for indented-code-blocks (Q2/A2) AND extended to Setext per the 2026-05-13 plan-stage finding F10, the wrapper does NOT filter Setext entries. Outline entries for Setext-style headings carry the level implied by the underline character (`====` → level 1, `----` → level 2) and the source line number of the text line (NOT the underline line).
...
- **FR-013**: The outline detector defers entirely to the upstream CLI / Obsidian metadata cache for heading recognition (per the 2026-05-13 plan-stage finding F10). Whatever the upstream parser returns IS the outline — both ATX-style headings (`#` through `######` followed by a space and at least one non-space character) and Setext underline-style headings (a text line followed by `====` for H1 or `----` for H2) appear as outline entries with their respective depths. The wrapper MUST NOT filter, drop, re-parse, or re-classify entries based on their syntactic form. (Plan-stage live probe: `obsidian outline format=json` against a fixture containing both forms returned both. The wrapper-side filter alternative was rejected because it would require a second invocation to read file content, defeating the single-call-per-request architecture.)
...
- **FR-023**: The feature MUST run a live-CLI characterisation pass before ship that documents observable CLI behaviour for: specific-mode happy path with multi-level headings; specific-mode with zero headings; specific-mode with level-skipping; specific-mode with fenced-block-containing-`#`-lines; specific-mode with indented-code-block-containing-`#`-lines (verifies the deferred-to-upstream contract per FR-012a and the 2026-05-13 clarifications session); specific-mode with closing-ATX-form heading (`## Heading ##`); specific-mode with inline-Markdown-rich heading text; specific-mode with Obsidian-anchor-in-heading; specific-mode with literal `::` in heading text; specific-mode with Setext-underline-style headings (verifies the deferred-to-upstream contract per FR-013 and the 2026-05-13 plan-stage finding F10 — Setext headings ARE included by upstream); specific-mode with CRLF and LF line endings (verifies `line` correctness across both); specific-mode count-only mode; specific-mode file-not-found; specific-mode unknown-vault response shape; specific-mode with non-`.md` filetype locator (verifies the FR-027 rejection contract for `.canvas`, `.pdf`, and at least one other Obsidian-recognised non-Markdown extension); active-mode with focused note; active-mode with no focus; active-mode with non-`.md` focused tab (verifies the FR-027 rejection covers active-mode parity); very-large-outline cap-boundary behaviour. Findings MUST be persisted in the feature's research artefact.
...
- **Setext inclusion (plan-stage finding)**: heading recognition defers to upstream (FR-013, amended at plan stage 2026-05-13 per finding F10). Setext-underline-style headings ARE included in the outline because the upstream CLI emits them. The original spec assumption (ATX-only, matching BI-015 precedent) was overridden when live probe revealed the upstream behaviour; the wrapper preserves whatever upstream emits to maintain the single-call-per-request architecture.
