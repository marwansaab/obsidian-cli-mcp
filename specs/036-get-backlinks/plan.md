...
8. **Live-CLI characterisation (T0 pass per FR-028 amended 2026-05-17)** — exercise the four load-bearing architectural probes against `TestVault-Obsidian-CLI-MCP` (per the `.memory/test-execution-instructions.md` protocol — read before any live-CLI invocation): F1 upstream `obsidian backlinks` subcommand shape (gates R1 eval-vs-native); F2 unknown-vault behaviour via eval (confirms R5 011-R5 cohort placement); F3 frontmatter-link inclusion in `getBacklinksForFile` (gates R6 per-source count semantic); F4 code-block exclusion (gates R8 defer-to-upstream). The remaining per-shape verification cases are validated at unit-test level in handler.test.ts against synthetic eval-envelope fixtures whose shape mirrors Obsidian's documented `LinkCache` and `CustomArrayDict` API contract (BI-025 precedent — see Analyze-stage Correction 1 in [spec.md](spec.md)). Persist live-probe findings in `research.md` under a `## T0 Live-CLI Capture (yyyy-mm-dd)` section; if any probe surfaces a TRIGGER deviation from eval-cohort precedent, pause and update spec / plan before continuing.
