...
- Q: How should the wrapper handle the boundary between the inherited 10-second typed-tool timeout (`TYPED_TOOL_TIMEOUT_MS = 10_000` in `src/cli-adapter/cli-adapter.ts`) and the embed-call latency variance — local models (~50-300ms) vs cloud models (typically 100-500ms but occasionally 5-10s+ under rate-limiting, network instability, or cold-cache)? → A: Option A — **defer to the cli-adapter's 10-second cap; document the boundary as an inherited limitation**. The `TYPED_TOOL_TIMEOUT_MS` constant is a project-wide invariant the eval-driven cohort already inherits; making `smart_connections_query` special-case violates that invariant. A query whose embed call exceeds 10 seconds surfaces as `CLI_TIMEOUT` (the standard adapter-layer error inherited per FR-013), NOT as `SMART_CONNECTIONS_NOT_READY(embed-failed)`. Agents reading `CLI_TIMEOUT` on this tool are directed by docs to suspect embed-model slowness as the most likely cause; the remediation path is "switch to a local embed model" or "wait and retry if rate-limited", not "the plugin is misconfigured". Locks: (a) new inherited limitation #7 added to FR-022 explicitly naming the 10-second boundary and the `CLI_TIMEOUT` surface; (b) addition to docs/tools surface explaining that `CLI_TIMEOUT` on this tool most commonly indicates embed-model latency; (c) no in-eval timeout race against the typed-tool budget (option B rejected); (d) no routing through `invokeBoundedCli` (option C rejected — preserves the typed-tool invariant).
- Q: How should the wrapper handle a `lookup` result whose source file no longer exists in the vault (the plugin's embedding index is stale — note deleted but still indexed until the next indexing pass)? → A: Option A — **pass through unchanged; document as inherited limitation #8**. The plugin's index is the source of truth; if it's stale, that's a plugin-state concern, not a wrapper concern. The wrapper does NOT pre-validate the match path with `app.vault.getAbstractFileByPath(path)`. Agents that need to verify path validity chain `read` against the match's path and observe the structured `FILE_NOT_FOUND` error. Parity with BI-026 (which also does not pre-validate match paths). Locks: (a) new inherited limitation #8 added to FR-022 (stale-index reverse direction — deleted notes still appearing in matches until next indexing pass); (b) new Edge Cases entry under CONTENT — match shape edge cases ("A match whose `path` references a file that no longer exists in the vault: pass through unchanged"); (c) NO additional in-eval filter (option B rejected — preserves the cross-mode count invariant FR-006a; preserves O(1) per-match work); (d) NO `verified: boolean` field on the public output shape (option C rejected — preserves the strict three-field `{path, headingPath, score}` contract from FR-007 and the grilling Q4 lock).
- A: **(live-probe-driven amendment 1, 2026-05-15 plan stage)** Live probe against "The Setup" vault (`obsidian "vault=The Setup" eval "code=app.plugins.plugins['smart-connections'].env.smart_blocks.lookup.toString()"`) revealed that `hypotheticals` is destructured at the **TOP LEVEL** of the params object, NOT inside `filter`. Source: `async lookup(params = {}) { const { hypotheticals = [] } = params; ... }`. The grilling-session Q3 assumption that the call shape is `lookup({ filter: { hypotheticals: [query], limit }, collection: 'smart_blocks' })` is FALSE. Corrected shape: `lookup({ hypotheticals: [query], filter: { limit }, collection: 'smart_blocks' })`. Locks: rewrite of FR-011 (`hypotheticals` at top level of params, NOT inside `filter`); the `filter` object still carries `limit` (verified: `filter.limit` IS honoured — `filter:{limit:3}` returned exactly 3 results); no other FRs change because the in-eval call site is the only place this shape matters. Plan-stage research.md F3 records the verified shape.
- A: **(live-probe-driven amendment 2, 2026-05-15 plan stage)** Live probe revealed two additional behaviours not anticipated by the spec: (a) lookup ERRORS return as a sentinel object `{ error: <string> }` from `lookup`, NOT as thrown exceptions or rejected promises (verified: empty `hypotheticals: []` → `{ error: "hypotheticals is required" }`; missing embed model → `{ error: "Embedding search is not enabled." }`). The in-eval pipeline MUST check `if (r && r.error)` after awaiting `lookup` and map to the appropriate envelope error code. (b) The CLI captures plugin-side stdout BEFORE the `=> ` eval return marker — the plugin's internal `console.log("Found and returned N smart_blocks.")` AND warning lines (`[warn] hypotheticals is required`) appear on stdout BEFORE the `=> ` line carrying the JSON return value. Locks: rewrite of FR-013a's "embed-failed" detection — the wrapper detects via `r.error` return-value inspection, NOT via in-eval try/catch (option B in the grilling Q5 chain — "lookup throws synchronously" — is FALSE for this plugin version; the actual mechanism is `{error}` return). Locks: handler parse-stage protocol — the JSON-extraction step MUST locate the LAST `=> ` occurrence (or split-on-`=> ` and take the last segment), NOT trimStart-and-slice-from-the-beginning. The BI-026 handler's `if (stdout.startsWith("=> ")) stdout = stdout.slice(3)` pattern works only because BI-026's `find_connections` API does NOT emit plugin-side console output to stdout — it's a quieter API. The new handler protocol applies only to BI-027 (and any future eval-driven tool that triggers plugin-side console output); BI-026's handler stays as-is.
...
- A match whose `path` references a file that no longer exists in the vault (plugin's index is stale — note deleted but still indexed until next pass): pass through unchanged. The wrapper does NOT pre-validate the match path. Agents needing path-existence verification chain `read` against the path and observe `FILE_NOT_FOUND`. Inherited limitation #8.
...
- **FR-011**: The JS template executed inside `eval` MUST reach the Smart Connections plugin's lookup API via the path `app.plugins.plugins["smart-connections"].env.smart_sources.lookup(...)`. The template MUST call lookup as `lookup({ hypotheticals: [query], filter: { limit }, collection: "smart_blocks" })` per the World-A live-probe-verified shape (research.md F3). The `hypotheticals` field lives at the TOP LEVEL of the params object (NOT inside `filter`) per plan-stage live-probe amendment 1. The `filter.limit` placement IS honoured by the plugin (verified — `filter:{limit:3}` returned exactly 3 results). The template MUST detect lookup errors via the sentinel `{ error: <string> }` RETURN VALUE (not via try/catch — the plugin returns the sentinel rather than throwing) per plan-stage live-probe amendment 2.
...
- **FR-013**: The tool's failure-mode roster MUST consist of EXACTLY the following structured errors (zero new top-level codes; zero new `details.code` strings — all already exist in the BI-026 roster):
  ...
  6. `CLI_REPORTED_ERROR` with `details.code = "SMART_CONNECTIONS_NOT_READY"` and `details.reason = "embed-failed"` — the plugin's lookup API IS callable BUT returns the sentinel `{ error: <string> }` when invoked (embedding model not loaded — verified live-probe message `"Embedding search is not enabled."`; invalid OpenAI API key, network failure for cloud embed models, rate-limited — these surface as additional error strings the lookup helper produces). Detected via in-eval `if (r && r.error)` return-value inspection per plan-stage live-probe amendment 2. The upstream error string is carried verbatim in the envelope's `detail` field.
...
- **FR-022**: The docs file MUST document the following inherited limitations:
  ...
  7. Embed-call latency cap — typed-tool calls inherit the cli-adapter's 10-second timeout (`TYPED_TOOL_TIMEOUT_MS` in `src/cli-adapter/cli-adapter.ts`). When the embed model the user has configured exceeds this budget (cloud-embed rate-limiting, network instability, cold-cache responses), the call surfaces as `CLI_TIMEOUT` — NOT as `SMART_CONNECTIONS_NOT_READY(embed-failed)`. Agents observing `CLI_TIMEOUT` on this tool should suspect embed-model latency as the primary cause; the remediation path is "switch to a faster embed model" or "retry after a brief delay" rather than "fix the plugin configuration".
  8. Stale-index reverse direction — notes deleted from the vault since the plugin's last indexing pass may still appear in lookup results until the next pass refreshes the index. The wrapper does NOT pre-validate match paths; agents needing to verify a match's source file exists chain `read` against the path and observe `FILE_NOT_FOUND` if the file has been deleted. Parity with limitation #2 (forward direction — newly-created notes don't yet appear).
...
- The Smart Connections plugin's `env.smart_sources.lookup` method exists on the live installed plugin version (LIVE-VERIFIED at plan stage 2026-05-15 against "The Setup" vault — F1).
- `hypotheticals` is the correct field name for text-to-embed-and-query input AND lives at the TOP LEVEL of params, NOT inside `filter` (LIVE-VERIFIED via `smart_blocks.lookup.toString()` source inspection at plan stage — F3 / plan-stage live-probe amendment 1).
- `filter.limit` is the correct placement for the cap (LIVE-VERIFIED — `filter:{limit:3}` returned exactly 3 results — F4).
- The plugin embeds query text internally via `embed_model.embed_batch(hypotheticals.map(h => ({embed_input: h})))`; the wrapper does NOT need to pre-compute embeddings (World A LIVE-VERIFIED — F3).
- `collection: "smart_blocks"` returns block-level keys in the same `path#h1#h2#{n}` format as BI-026's `find_connections` (LIVE-VERIFIED — example key `"425-Skills/Skill Creator.md#Skill Creator#Gotchas#{1}"` — F5 / F6).
- Closed-but-registered vault behaviour reproduces on `lookup`-based eval calls with the same empty-stdout + transparent-open signature as on `find_connections`-based calls (deferred to T0 of `/speckit-implement` — requires user to close a vault mid-session; the shared `_eval-vault-closed-detection` module is plugin-agnostic per FR-020 so the signature is expected to reproduce).
- The plugin's lookup API failures surface as `{ error: <string> }` return-value sentinels, NOT as thrown exceptions or rejected promises (LIVE-VERIFIED — empty hypotheticals → `{error: "hypotheticals is required"}`; missing embed model → `{error: "Embedding search is not enabled."}` — F12 / plan-stage live-probe amendment 2).
- Plugin-side console output (warnings, info lines like `"Found and returned N smart_blocks."`) is captured by the CLI on stdout BEFORE the `=> ` eval return marker; handler stage-1 parse MUST extract the JSON from the LAST `=> ` occurrence, NOT from the beginning of stdout (LIVE-VERIFIED — plan-stage live-probe amendment 2).
