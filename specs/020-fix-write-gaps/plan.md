...
**Testing**: vitest with `@vitest/coverage-v8`. Co-located `*.test.ts` per module. The aggregate statements coverage threshold floor is set in [vitest.config.ts](../../vitest.config.ts). Tests inject `deps.spawnFn` per the existing test-seam convention; no real `obsidian` binary executions in CI. New handler tests use `nodeFs.writeFile` mocking and `nodeFs.realpath` mocking via dependency injection per the existing 016 handler-test pattern. Audit-confirmation T005 verified zero existing `handler.test.ts` cases use the `file` parameter — the eight new tests are pure additions (no updates to existing cases needed).
...
**Coverage gate**: aggregate statements floor is set in [vitest.config.ts](../../vitest.config.ts). The handler edit is a tiny diff; the eight new test cases (pure additions per the T005 audit-confirmation — no existing cases need updating) provide near-100% coverage of the edited code paths, so the aggregate either stays flat or ratchets up. No coverage drop expected; final figure verified in the post-implement quality gate.
...
│   │   ├── handler.test.ts                          # EDITED — +~80 LOC for 8 NEW test cases per FR-022's symmetric coverage rule (no updates to existing cases — T005 audit-confirmation verified zero existing `file:`-parameter cases)
│   │   ├── index.ts                                 # FROZEN
...
- **R9 — Test surface**: eight NEW handler test cases. Audit-confirmation T005 verified (via `grep -n "file:" handler.test.ts` returning empty) that zero existing test cases pass the `file` parameter — every existing case uses `path: "..."` exclusively. The short-form rule therefore introduces no broken-expectation cases to update; the eight new tests are pure additions.
