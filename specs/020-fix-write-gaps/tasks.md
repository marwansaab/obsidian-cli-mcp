...
- [X] T007 [P] [US1] Add **one** path-form regression-guard handler test case to [src/tools/write_note/handler.test.ts](../../src/tools/write_note/handler.test.ts) per [data-model.md test inventory case #5](./data-model.md): `path: "Subfolder/Note.md"` → response `path: "Subfolder/Note.md"` verbatim (no double-extension, no `.md` re-append, no behavioural change from current 016 behaviour). Covers FR-004 / Story 1 AC#4 / SC-003.
...
- [X] T012 [P] Update [docs/tools/write_note.md](../../docs/tools/write_note.md) per FR-018 / R14 with **two** short callouts under existing sections:
