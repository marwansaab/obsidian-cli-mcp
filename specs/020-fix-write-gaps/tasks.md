...
**Tests**: REQUIRED per Constitution Principle II — every public-surface modification ships with happy-path AND failure-or-boundary tests in the same change. Tests are co-located with sources (`*.test.ts` next to `*.ts`); the test inventory is locked at 8 new / updated handler cases per [data-model.md test inventory](./data-model.md).
...
- [ ] T001 Run `npm run test` and `npm run typecheck` against the 020-fix-write-gaps branch HEAD and record the pre-fix passing-case count for [src/tools/write_note/handler.test.ts](../../src/tools/write_note/handler.test.ts) (the file that gains the eight new / updated cases in Phase 3 + Phase 4). Confirm both commands exit zero. This establishes the green baseline against which the fix's regression guards (T005, T010, T011) operate.
...
- [ ] T005 [US1] Audit existing handler.test.ts cases that assert against short-form `file` input (e.g. cases that called `file: "Acceptance Probe"` and expected the on-disk path to be `Acceptance Probe` extension-less, or that asserted `response.path === "Acceptance Probe"`). Update those cases to assert the new resolved expectations per the FR-001 rule (response.path is `<file>.md`; on-disk path ends in `<file>.md`). Identification method: after T004 lands, run `npm run test`; any failing handler.test.ts case whose failure is in the relPath / response.path assertion is in-scope for this update. Depends on T004.
