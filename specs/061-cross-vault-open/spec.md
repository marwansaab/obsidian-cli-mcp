...
- **Enabling mechanism — a single vault-targeted eval (B1 falsified)**: A 2026-06-01 forcing-gate probe **falsified upstream limitation B1** — `eval` honours `vault=` and routes to the named vault (so BI-057's focused-vault guard guarded against a limitation that does not exist). **Resolved (ADR-031)**: `open_file` issues one vault-targeted eval — `invokeCli({command:"eval", vault:requested, target_mode:"specific", code})` — which **runs in the requested vault**, resolves the locator there, and opens the file (switching focus to that vault) via the explicit placement branch. No focused-vault guard, no `obsidian://` focus-switch, no verify-poll, no `launchObsidian` import. Native `open`/`tab:open` also honour `vault=` but open in the active leaf and **cannot focus an existing tab**, so they cannot deliver `existing_tab_reused`; **OQ-1 is resolved against native**. The cohort-wide B1 re-verification is tracked in **BI-0134** (`open_file` excluded). See [plan.md](plan.md), [research.md](research.md), and [contracts/t0-probe-findings.md](contracts/t0-probe-findings.md).
- **Closed-vault / app-down recovery is fully inherited at the dispatch chokepoint (ADR-029 / ADR-030)**: The "transient first-attempt failure recovered automatically" (FR-005) is provided entirely by the shared dispatch chokepoint, **vault-correctly** because the eval carries `vault=requested`: the cold-start retry (ADR-029 / BI-059) recovers a closed-but-registered vault (attempt-1 `COLD_START_PATTERN`), and the application-launch recovery (ADR-030 / BI-060) launches `obsidian://open?vault=requested` for the app-down case (no default-vault detour, no extra round). The feature adds **no** bespoke launcher and **no** per-tool retry. The ADR-029 FR-013 "wrong vault focused" residual does not arise for a vault-targeted eval. The `OBSIDIAN_AUTO_LAUNCH` opt-out is honored; the bound is the inherited mechanisms' bound.
...
- **ADR-029 — Retry Once on Cold-Start Vault-Launch Failure** (Decided) / **BI-059 — Retry Cold Start**: the dispatch-layer cold-start retry the closed-vault recovery inherits; its FR-013 "wrong vault focused" carve-out does not arise for a vault-targeted eval (B1 false → the eval routes to the requested vault directly); recovery is inherited, not re-implemented.
