...
- **Closed vault never reaches a ready state within the recovery bound**: the bounded recovery terminates and surfaces a distinct "vault could not be opened" error (composing with the application-down recovery of BI-060), never an indefinite hang and never a fabricated success.
- **Obsidian application is entirely closed (no process running)**: bringing up the requested vault composes with the application-launch recovery (BI-060 / ADR-030), which opens Obsidian focused on the requested vault via the vault-targeted URI; the file then opens there. If the application cannot be brought up, the distinct error of BI-060 surfaces.
- **Unknown / unregistered vault**: surfaces as the vault-unknown error, distinct from the closed-but-openable case (which is now a success path) and from the recovery-exhausted case.
...
- **FR-002**: System MUST be able to open a file in a vault that is currently closed but registered with Obsidian, bringing that vault up and switching focus to it so the file surfaces, without requiring any person to manually open the vault.
...
- **FR-005**: When opening in a closed (cold) vault, System MUST automatically recover a transient first-attempt failure caused by the vault's bring-up / cold-launch window, returning the real open result on the single call the caller made — without the caller issuing a retry. This recovery MUST compose with (not duplicate) the existing cold-start retry (ADR-029 / BI-059) and the application-launch recovery (ADR-030 / BI-060). It MUST be bounded: at most a bounded recovery effort, then terminate with a distinct error (FR-016) rather than hang or loop indefinitely.
- **FR-006**: System MUST own the recovery for the open's cold-start manifestation. The dispatch-layer cold-start retry (BI-059 FR-013) explicitly excludes the eval-composed-envelope manifestation that this tool produces, so the transient-failure recovery for cross-vault opens is this feature's responsibility, not the shared dispatch retry's.
...
- **FR-013**: When the requested vault is not registered / not known to Obsidian at all, System MUST fail with a typed error that identifies the vault as unknown/unregistered, open nothing, and be programmatically distinguishable from the closed-but-openable case (now a success path) and from the recovery-exhausted case (FR-016).
- **FR-014**: When the requested vault is valid (focused, open, or recoverable) but the requested file does not exist in it, System MUST fail with a file-not-found error that names the requested location and open nothing — never a fabricated success.
- **FR-015**: System MUST keep the vault-error and file-error modes programmatically distinguishable from one another: unknown-vault (FR-013), file-not-found (FR-014), and recovery-exhausted (FR-016) are distinct outcomes a caller can branch on.
- **FR-016**: When a registered vault is closed (or its application is down) and cannot be brought to a ready state within the recovery bound (FR-005), System MUST fail with a distinct "vault could not be opened" error, programmatically distinguishable from the unknown-vault error (FR-013) and from a success. This error composes with the application-down signal of BI-060.
...
- **FR-018**: System MUST introduce no new top-level error code. Every failure reuses an existing top-level code with `details` sub-discrimination per ADR-015, preserving Constitution Principle IV's zero-new-top-level-codes streak. The exact code/reason literals (including how unknown-vault, recovery-exhausted, and the retired BI-057 "registered-but-not-focused" reason are encoded for this tool) are pinned at plan-phase.
...
- **SC-009**: When a registered vault cannot be brought to a ready state within the recovery bound, the operation terminates and surfaces a distinct error in bounded time in 100% of such cases — it never hangs, loops, or fabricates success.

## Assumptions

- **Extends `open_file` (BI-057), and deliberately supersedes its no-switch contract**: This feature builds on the existing `open_file` tool. It supersedes BI-057 FR-010 (the open "MUST NOT switch to, focus, or open a different vault") and FR-011 (the active focused-vault guard that errored with `VAULT_NOT_FOUND` / `details.reason: "not-open"` when the requested vault was not focused). Under this feature, "registered but not focused" and "closed but registered" become **success** paths (switch focus / bring up), and only "unknown/unregistered" remains a vault error. Per the project's supersede-don't-drift rule (CLAUDE.md, ".decisions/"), this contract inversion is a deliberate architectural event that MUST be recorded in a new ADR at plan-phase rather than silently overriding BI-057; the BI-057 `details.reason: "not-open"` semantics are retired or repurposed for this tool (the recovery-exhausted case of FR-016 is its natural successor). This assumption surfaces the conflict explicitly; the ADR is authored in the plan phase.
- **Enabling mechanism — vault-targeted focus (overcoming upstream limitation B1)**: Opening in a non-focused vault is only possible because the project already has a mechanism to bring a *specific* vault to focus. The eval substrate ignores `vault=` and always runs against the focused vault (upstream limitation B1, `.architecture/Obsidian CLI - Upstream Issues and Limitations.md`); BI-057's guard existed precisely because of B1. ADR-030 (BI-060) shipped a vault-targeted `obsidian://open?vault=<name>` URI opener (`app-launcher`) that opens/focuses a named vault. The plan phase confirms how this feature reaches a non-focused or closed vault — reusing the ADR-030 vault-targeted URI (optionally with a `&file=` target), and/or a focus-switch step before the open eval — and MUST respect ADR-030's "exactly two sanctioned spawn sites" invariant (no third spawn site).
- **Closed-vault recovery composes with existing recovery (ADR-029 / ADR-030)**: The "transient first-attempt failure recovered automatically" (FR-005) sits on top of, and does not re-implement, the cold-start retry (ADR-029 / BI-059) and the application-launch recovery (ADR-030 / BI-060). Because BI-059 FR-013 carves the eval-composed-envelope cold-start manifestation out of the dispatch-layer retry, this tool owns its own recovery for that manifestation (FR-006). The bound and the exact recovery shape are pinned at plan-phase against the authorised test vault.
...
- **Error vocabulary — zero new top-level codes (Constitution Principle IV)**: Every failure reuses an existing top-level code with `details` sub-discrimination per ADR-015 (FR-018). Settled at spec time: unknown-vault, file-not-found, and recovery-exhausted are mutually distinguishable, and the recovery-exhausted case composes with BI-060's application-down `details.reason`. The exact code/reason literals are pinned at plan-phase.
...
- **ADR-030 — Sanctioned App-Launch Spawn Site** (Decided) / **BI-060 — Recover Closed Obsidian**: provides the vault-targeted `obsidian://open?vault=` URI opener and the two-sanctioned-spawn-site invariant the cross-vault focus mechanism reuses and must respect.
- **ADR-029 — Retry Once on Cold-Start Vault-Launch Failure** (Decided) / **BI-059 — Retry Cold Start**: the dispatch-layer cold-start retry the closed-vault recovery composes with; its FR-013 carve-out of the eval-envelope manifestation is what makes this tool's own recovery (FR-006) necessary.
- **ADR-015 — Sub-Discriminators via `details.reason`** (Decided): the vehicle for distinguishing unknown-vault, recovery-exhausted, and file-not-found without a new top-level error code.
