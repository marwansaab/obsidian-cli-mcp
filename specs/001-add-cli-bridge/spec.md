...
- Q: How should the bridge respond to OS-level termination signals (SIGINT, SIGTERM) delivered to the bridge process itself? → A: Treat them identically to MCP transport close — install handlers that run the same kill-in-flight + drop-queue + emit-shutdown-log cleanup as FR-028, then exit with code 0. The `bridge.shutdown` log line's `reason` field discriminates: `"transport_closed"` vs `"signal:SIGINT"` vs `"signal:SIGTERM"`. Hard kills that bypass Node's signal handlers (e.g., `taskkill /F`, `kill -9`) cannot be cleaned up — that limitation is inherent to the OS, not a spec defect.
...
- **Operator sends SIGINT or SIGTERM to the bridge process** (Ctrl+C in a foreground shell, `Stop-Process`, `taskkill` without `/F`, parent-process kill): the bridge runs the same cleanup as the transport-close path — kill the in-flight child, drop the queue, emit a final `bridge.shutdown` log line with `reason: "signal:SIGINT"` or `reason: "signal:SIGTERM"`, then exit with code 0. Hard kills (`taskkill /F`, `kill -9`) bypass Node's signal handlers and CANNOT be cleaned up; orphan children in that case are an OS-level limitation, not a spec defect.
...
- **FR-028**: When the bridge is asked to shut down — either because (i) the MCP transport closes (stdin EOF, client disconnect, or transport error), or (ii) the bridge process receives SIGINT or SIGTERM — the bridge MUST: (a) send SIGTERM to the in-flight `obsidian` child if one is running, (b) send SIGKILL after a 2-second grace period if the child has not exited, (c) discard every queued call without spawning it, (d) emit the FR-029 shutdown log line, and (e) exit the bridge process cleanly with exit code 0. Both shutdown triggers MUST run identical cleanup. Dropped queued calls do NOT need to surface an `UpstreamError` (the receiver is gone or the operator has signaled abort). Orphan child processes from any cleanly-signaled shutdown are a defect; orphans from hard kills (`taskkill /F`, `kill -9`) that bypass Node's signal handlers are an OS-level limitation outside the bridge's control.
- **FR-029**: The bridge MUST emit a final shutdown log line to stderr before exiting — `{ "event": "bridge.shutdown", "ts": <ISO-8601>, "reason": <"transport_closed"|"signal:SIGINT"|"signal:SIGTERM">, "inFlightKilled": <boolean>, "queuedDropped": <integer> }` — so the operator can audit shutdown causes (and distinguish transport-driven from signal-driven shutdowns) from the log stream.
...
- **Bridge process**: the long-lived Node.js process running on the Windows host that hosts the MCP server, owns the stdio transport, maintains a FIFO queue of pending invocations, and spawns one short-lived `obsidian` child per invocation. Lifetime is bound to (a) the MCP transport — when stdin closes, the bridge kills the in-flight child, drops the queue, and exits — and (b) OS termination signals (SIGINT, SIGTERM) delivered to the bridge process itself, which run the same cleanup. Hard kills bypass cleanup.
