...
### Plan-stage corrections (2026-05-17)

The /speckit-plan phase produced two corrections to spec-stage Assumptions. Both are recorded here for traceability; the user-facing contract (FRs, SCs, edge cases) is unchanged.

- **Correction 1 (tool name)**: the spec-stage Assumption named the new tool `search_context` (underscore-flatten of the colon). Plan research R2 (see [research.md](research.md)) applies ADR-010 strict: composite `namespace:action` upstream subcommand → `<action>_<namespace>` tool name. Upstream is `obsidian search:context` (namespace `search`, action `context`); the reversal is **`context_search`**. Parallels the existing project precedents `read_property` (`property:read`) and `set_property` (`property:set`). The spec-stage Assumption is restated below to reflect the correct name. Driver: ADR-010 is a binding Constitution Compliance gate; the rule fires the same way for every composite upstream subcommand.
- **Correction 2 (folder-existence-check mechanism)**: the spec-stage Assumption flagged the FR-013 folder-existence-check mechanism for `/speckit-plan` to decide. Plan research R4 resolves it: **post-empty probe via `obsidian folder path=<normalised-folder>`** — only invoked when `search:context` returns the zero-match sentinel AND `folder` was supplied. Live-probe F3 (2026-05-17) confirmed `obsidian folder path=NonExistentFolder` returns stdout `Error: Folder "X" not found.` with exit 0, which the dispatch-layer classifier in `_dispatch.ts:308-318` priority (c) catches and emits as `CLI_REPORTED_ERROR` for free — no new top-level code, no new `details.code` value, no wrapper-side classification step. Driver: cheaper than pre-flight on the happy path (1 CLI call vs 2); inherits structured-error envelope from existing infrastructure.
...
- The new registered tool name is **`context_search`** per ADR-010 strict reading (composite `namespace:action` upstream subcommand → `<action>_<namespace>` reversal — see [research.md](research.md) R2 and Clarifications Plan-stage Correction 1 2026-05-17). Upstream is `obsidian search:context` (namespace `search`, action `context`); the reversal is `context_search`. Parallels the project's existing composite-subcommand wrappers `read_property` and `set_property`. The source module is at `src/tools/context_search/` with factory function `createContextSearchTool`.
...
- The folder-existence check (FR-013) is implemented as a **post-empty probe via `obsidian folder path=<normalised-folder>`** — only invoked when `search:context` returns the zero-match sentinel AND `folder` was supplied (see [research.md](research.md) R4 and Clarifications Plan-stage Correction 2 2026-05-17). Live-probe F3 (2026-05-17) confirmed the inherited dispatch-layer classifier in `_dispatch.ts:308-318` priority (c) catches the upstream `Error: Folder "X" not found.` stdout sentinel and emits `CLI_REPORTED_ERROR` for free — no new top-level code, no new `details.code` value, no wrapper-side classification step. The check fires wrapper-side (post-search) rather than pre-flight to keep the happy-path cost at one CLI call.
