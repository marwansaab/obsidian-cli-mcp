...
- **FR-003**: The tool MUST accept an optional `folder` parameter (vault-relative path prefix). When supplied, only files whose vault-relative path begins with that prefix MUST appear in the response. Matching MUST be at folder-segment boundaries (`folder=Proj` MUST NOT match a file under `Projects/`).
...
- **FR-020**: The tool's progressive-disclosure help surface MUST document the full input contract, the output shape, the error roster, and at least four worked examples. The help surface MUST also include a one-sentence guidance line indicating when to prefer this tool over the path-only `search` sibling (and the `search` help surface SHOULD carry the inverse pointer — guidance to prefer the context tool when per-line context is needed; the `search` update is a documentation touch, not a code-behaviour change).
...
- **SC-003**: A `folder=F` query returns 100% of matches under `F/` and 0% of matches outside `F/`. A `folder=F` query against a folder that does not exist surfaces a structured folder-not-found error, NOT `count=0`.
...
- **SC-009**: A line whose raw text exceeds 500 characters appears in the response with `text` truncated to 500 characters plus a single `…` (U+2026) marker; lines ≤ 500 characters are returned verbatim with no ellipsis.
...
- The existing `search` tool retains its current input and output surface unchanged by this BI. Its existing `context_lines` boolean flag (which today switches the response between path-only and line-context shapes per BI-033 FR-004) becomes REDUNDANT after this BI ships and is a candidate for a future deprecation BI — but is NOT modified here. The non-breaking add-then-deprecate path is preferred over an in-place split because: (a) shipped callers depending on `search` + `context_lines=true` continue to work; (b) the help-surface guidance line (FR-020) directs new callers to the dedicated tool; (c) a future deprecation BI can run a clean removal window. This is a deliberate design trade — flagged for `/speckit-plan` to confirm before locking in.
...
- Modifying the existing `search` tool's input or output surface. The `context_lines` flag on `search` is left in place; its future deprecation is a separate BI.
