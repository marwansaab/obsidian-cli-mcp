...
> **Cap supersession note (2026-05-27)**: Every reference in this spec to "24576 characters" or "10240 ASCII characters" as the documented cap is **superseded by FR-008a** (new cap: **3072 characters**). The supersession is grounded in the T0-R1 clean-bisect probe (10/10 trials succeed at 3584; 0/10 at 4096 — the upstream Obsidian.com argv-IPC defect on Windows). The "approximately 10 KB" wording in the Input section is also empirically wrong — the actual threshold is ~4 KB. Where this spec uses 24576 / 24577 / 10240 as boundary numbers in FRs / SCs / Acceptance Scenarios, read them as the cap-relative semantic ("at the cap", "one over the cap", "in-cap stress payload") with the current cap value coming from FR-008a + [`src/tools/prepend/schema.ts:16-29 MAX_CONTENT_LENGTH`](../../src/tools/prepend/schema.ts). Updating every numeric occurrence inline would obscure the supersession audit trail; the supersession note is the durable record.
...
- **FR-008** *(SUPERSEDED 2026-05-27 by FR-008a — see below)*: The fix MUST preserve the published schema cap at 24576 characters. The cap MUST NOT be lowered as part of this change. The cap is enforced against the character count of the content payload (matching the published cap unit); the byte-length expansion of non-ASCII payloads under UTF-8 does NOT alter the enforcement threshold and is handled by the host-process stability requirement (FR-004) without a crash dialog. *(Operational cap unit: see [contracts/prepend-input.contract.md](contracts/prepend-input.contract.md) §"Cap unit reconciliation" — the schema enforces against `string.length`, which is the UTF-16 code-unit count per JavaScript's spec. For ASCII content the two units coincide; for non-BMP characters one Unicode character takes 2 UTF-16 code units.)*
- **FR-008a** *(supersedes FR-008, added 2026-05-27 after T0-R1 probe)*: The published schema cap is lowered from 24576 to **3072** characters in this BI. The "MUST NOT be lowered" clause in FR-008 is overridden by the empirical finding from the T0-R1 clean-bisect probe (raw evidence under [specs/047-fix-prepend-reliability/.scratch/t0-r1-bisect/](.scratch/t0-r1-bisect/) — local-only): the upstream `Obsidian.com` console launcher's IPC channel to the running `Obsidian.exe` GUI breaks deterministically at content ≥ 4096 bytes on Windows. 10/10 trials succeed at 3584; 0/10 at 4096 — calls SIGTERM at 12 s and Obsidian's CLI-receiving state degrades until the GUI is restarted. The cap of 24576 was therefore never achievable in practice; lowering to 3072 is the honest published contract. 3072 leaves ~1 KB headroom for `path=` / `vault=` argv overhead a real prepend call adds on top of content. The constant `MAX_CONTENT_LENGTH` in [src/tools/prepend/schema.ts](../../src/tools/prepend/schema.ts) is the single source of truth; the tool description string and the published `inputSchema.properties.content.maxLength` both reflect 3072 after this BI lands. Callers needing larger payloads use the full-replace `write_note` surface (fs-direct, cap-free). The cap MAY be ratcheted back up in a future BI if upstream Obsidian repairs the argv-IPC defect (forum thread 113867 — no Obsidian-team response as of BI-045 evidence).
...
- **SC-002**: A regression run of 50 consecutive prepend calls against 50 different target notes in a registered vault, each with a content payload of exactly 10240 ASCII characters (10 KiB), completes with 0 silent no-ops (silent no-op as defined by SC-005 — a success envelope of any shape when the target note's on-disk byte count is unchanged), 0 wrapper timeouts, and 0 Obsidian host-process crash dialogs across the full sequence.
...
- Active-mode prepend failures (the deterministic "Vault not found" failure when no caller-supplied locator is present) are tracked under a separate bug. The plan's Phase 0 R2 in [research.md](research.md) has SUPERSEDED this assumption with an empirical decision: the BI-0017 active-mode root cause (synchronous-non-priming `resolveVaultDisplayName` falling through to `parsed.base`) is **distinct from** the BI-047 large-content failure surface. The active-mode fix therefore does NOT land in the same change set; `src/vault-registry/registry.ts` remains untouched in this BI. The cross-BI tracking note is preserved here for historical context; the operative decision is in `research.md` R2.
