...
- [X] T001 Atomic flatten of `target-mode.ts` (rewrite + handler tweak + 8 test edits in one commit). Per [contracts/flat-target-mode.contract.md](contracts/flat-target-mode.contract.md) and [data-model.md](data-model.md) §2. FR-001 / FR-002 / FR-003 / FR-004 / FR-017.

  - **(1a) Rewrite [src/target-mode/target-mode.ts](../../src/target-mode/target-mode.ts) — schema layer.** Replace the `targetModeBaseUnion` discriminated union + `superRefine` dispatcher with three exports: `targetModeBaseSchema = z.object({...}).strict()` + `applyTargetModeRefinement<T extends z.ZodObject<z.ZodRawShape, z.UnknownKeysParam>>(s: T): z.ZodEffects<T>` + `targetModeSchema = applyTargetModeRefinement(targetModeBaseSchema)`. Inline the existing `refineSpecificBranch` / `refineActiveBranch` private functions into the helper's `superRefine` body, preserving per-issue `path` / `code` / `message` content verbatim per FR-002 / FR-003.
  - **(1b) Delete six per-mode exports** per FR-017 / clarification C2: `targetModeSpecificBaseSchema`, `targetModeActiveBaseSchema`, `targetModeSpecificSchema`, `targetModeActiveSchema`, `applyTargetModeSpecificRefinement`, `applyTargetModeActiveRefinement`. Also delete the deprecated `TargetModeSpecific` / `TargetModeActive` type exports. The `// Original — no upstream.` header at line 1 stays as-is.
  - **(1c) Migrate six test cases in [src/target-mode/target-mode.test.ts](../../src/target-mode/target-mode.test.ts)** per [data-model.md §6](data-model.md#§6--test-case-migration-map-fr-003--fr-017--r6): replace `targetMode{Specific,Active}Schema.safeParse({...})` with `targetModeSchema.safeParse({ target_mode: "<mode>", ...rest })`. The other 25 cases are unchanged. Per-issue assertions preserved verbatim per FR-003.
  - **(1d) Add two new co-located test cases** per [data-model.md §6 N1+N2](data-model.md#§6--test-case-migration-map-fr-003--fr-017--r6):
    - **N1 — strict-mode boundary** (R4 / FR-002 carve-out): `targetModeSchema.safeParse({ target_mode: "active", random: "x" })` rejects with one issue, `code: "unrecognized_keys"`, `keys: ["random"]`, `path: []`.
    - **N2 — extension happy-path** (R2): `applyTargetModeRefinement(targetModeBaseSchema.extend({ note_text: z.string() }))` accepts `{ target_mode: "specific", vault, file, note_text }` AND rejects an unknown top-level key (verifies `.extend()` preserves `.strict()`).
  - **(1e) Update [src/tools/read_note/handler.ts](../../src/tools/read_note/handler.ts)** at lines 21-27 with `input.vault!` non-null assertion paired with a single-line comment naming the `superRefine` runtime invariant (clarification C1).
  - **(1f) Verify [src/tools/read_note/schema.ts](../../src/tools/read_note/schema.ts)** — already a thin re-export of `targetModeSchema`; should not require edits, but verify the `TargetMode` import still resolves to the flattened type after T001.

  **Constitution**: Principle I (no new module; `target-mode/` does not gain a dependency on `tools/`); Principle II (tests in same change-set); Principle III (the trivial `zodToJsonSchema` derivation is the cleanest possible single-source-of-truth compliance); Principle V (header preserved).
