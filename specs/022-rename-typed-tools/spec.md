...
- Q: Internal-code rename scope — do `src/tools/<old>/` source directories and `createXxxTool` factory function names rename in lockstep with the registered tool name, or only the registered name changes? → A: **Rename in lockstep.** Source directories rename (`src/tools/read_note/` → `src/tools/read/`, `delete_note/` → `delete/`, `list_files/` → `files/`, `write_property/` → `set_property/`, `rename_note/` → `rename/`) AND factory function names rename (`createReadNoteTool` → `createReadTool`, `createDeleteNoteTool` → `createDeleteTool`, `createListFilesTool` → `createFilesTool`, `createWritePropertyTool` → `createSetPropertyTool`, `createRenameNoteTool` → `createRenameTool`). Upholds the path-matches-name convention every prior typed tool follows; eliminates path/name drift; aligns with FR-021's rationale that retaining factory names that name a retired tool would create drift between the registry and the source. Co-located test files (`schema.test.ts`, `handler.test.ts`, `index.test.ts`) migrate with their directories. Plan-stage decides the git-mv-vs-rewrite mechanics; this clarification locks the scope only.
- Q: FR-018 verification-test lifecycle — is the post-rename `tools/list`-vs-baseline test transient (added then removed in the rename branch), release-scoped (kept for one release then dropped), or permanent? → A: **Permanent registry-stability test.** The test ships as durable machinery: a snapshot-style assertion that the live registered tool list matches a checked-in JSON baseline at a stable path (plan-stage decides the exact path; expected under `src/tools/_register.test.ts` or a sibling). The baseline is intentionally rolled forward whenever a future BI adds, removes, or renames a tool — that BI's diff includes both the registry change and the baseline update in the same commit, making any unintended registry mutation visible at review time. Catches accidental renames in every future feature, not just this BI's intended five. Pre-v1.0 window is the cheapest moment to install the gate.
- Q: Internal cross-reference rewrite scope — beyond `README.md` and `docs/tools/*.md` (FR-012), which other in-repo files have tool-name references rewritten as part of this BI? → A: **Narrowest scope.** Rewrite tool-name references ONLY in `README.md`, `docs/tools/*.md`, and `CLAUDE.md`'s active-narrative block at the top (which describes the active 022 feature and naturally uses new names). NOT rewritten by this BI: ADR text in `.decisions/`, architecture docs in `.architecture/`, `CONTRIBUTING.md`, source-code comments anywhere in `src/`, and predecessor `specs/0XX-*/` files. Rationale: ADRs and architecture docs reference the tools by the names that were current when the ADR was written; rewriting them would falsify the historical record. Source-code comments rarely name tools by spelling them out, and sweeping them would balloon the diff with low-value churn. `CONTRIBUTING.md` is a contributor-workflow document, not a tool-catalogue document — any stale tool references there age along with the rest of the file. Future BIs that touch any of these files for other reasons MAY incidentally update tool-name references they encounter; this BI does NOT proactively sweep them.

### Session 2026-05-12 — /speckit-analyze remediations

Cross-artifact analysis ran post-tasks and produced 7 findings (0 CRITICAL, 0 HIGH, 2 MEDIUM, 5 LOW); all 7 were remediated in the same session by editing tasks.md / research.md / plan.md / this spec / the registry-baseline contract. No CRITICAL findings surfaced and no constitution alignment issues were found. Findings + their landing spots:

- **C1 (FR-015 coverage gap, MEDIUM)**: Catalogue listing — no explicit task verified that `help()` (no args) returns a list with new names and no retired names. Landed: tasks.md T025 body extended to include a `it("catalogue listing reflects the renamed registry", ...)` assertion alongside the five `help({ tool_name: <new> })` lookup cases.
- **C2 (Handler-name policy traceability, MEDIUM)**: The decision to NOT rename handler-internal function names (`executeReadNote` etc.) lived only in tasks.md task bodies; not traceable from spec or research. Landed: new research.md R11 decision with three-bullet rationale (collision avoidance vs `executeReadHeading` / `executeReadProperty`; not a public surface; smaller diff).
- **C3 (FR-008 / SC-009 automated check, LOW)**: "No new error codes" verified only by review + typecheck. Landed: tasks.md T032 body extended with `git diff main -- src/errors.ts` returns-empty assertion.
- **A4 (Ambiguity: "filetype-scope language", LOW)**: Phrase used in spec's Out of Scope without pinned definition. Landed: research.md R3 expanded with a concrete definition ("Markdown note" / "note"-as-filetype-anchor / ".md" mentions are in scope; "vault" / "file" / "path" / "frontmatter" / "heading" are out of scope and may be edited freely).
- **U5 (`.scratch/` gitignore assumption, LOW)**: T001 assumed `.scratch/` was already gitignored. Landed: T001 body extended to verify the line is present in `.gitignore` and add it via a single-line commit if absent.
- **U6 (Baseline-capture mechanism unpinned, LOW)**: T028 left the regeneration mechanism undecided. Landed: tasks.md T027 + T028 + contracts/registry-baseline.contract.md §5 + §7 all updated to specify a shared `src/tools/_register-baseline.ts` module consumed by both `_register.test.ts` and a new `scripts/write-register-baseline.ts` script wired as `npm run baseline:write`. The shared module gets its own co-located test file per Constitution Principle II.
- **I7 (Plan vs tasks ID inconsistency, LOW)**: Plan's Phase 2 outline used different task IDs than tasks.md's authoritative IDs. Landed: plan.md Phase 2 caveat updated to call the outline IDs "planning placeholders" superseded by tasks.md T001..T036.
...
**Independent Test**: An automated unit test (added as part of this BI as durable machinery per Clarifications Q2) that compares the live `tools/list` output against a checked-in JSON baseline. For the rename release the baseline is the post-rename five-name punch-list. The test fails if any of the following hold: (a) any retired name still appears in the registry, (b) any new name is missing from the registry, (c) any tool not in the punch-list has been renamed, or (d) any tool has been added/removed besides the punch-list. The test persists after the release ships; future BIs that intentionally change the registry roll the baseline forward in the same commit.
...
- **FR-009**: The renamed tools' implementation files MUST be moved to new paths matching the new names (per Clarifications Q1, 2026-05-12): `src/tools/read_note/` → `src/tools/read/`, `delete_note/` → `delete/`, `list_files/` → `files/`, `write_property/` → `set_property/`, and `rename_note/` → `rename/`. Co-located test files (`schema.test.ts`, `handler.test.ts`, `index.test.ts`) move with their directories. The move MUST NOT change the implementation's externally observable behaviour.
...
- **FR-018**: An automated unit test (permanent registry-stability gate per Clarifications Q2, 2026-05-12) MUST snapshot the live registered tool list against a checked-in JSON baseline and fail if the two diverge. For the rename release, the baseline reflects the post-rename five-name punch-list — the test fails if any retired name still appears, any new name is missing, any tool outside the punch-list has been renamed, or any tool has been added or removed beyond the punch-list. The test ships as durable machinery: future BIs that intentionally add, remove, or rename a tool MUST roll the baseline forward in the same commit as the registry change, so any unintended registry mutation is visible at review time.
...
- **FR-020**: Internal cross-references that point at a renamed tool MUST be updated to use the new name in this narrowly-scoped file set (per Clarifications Q3, 2026-05-12): `README.md`, `docs/tools/*.md`, and `CLAUDE.md`'s active-narrative block at the top (which describes the active 022 feature and naturally uses new names). Files NOT rewritten by this BI: ADR text in `.decisions/`, architecture docs in `.architecture/`, `CONTRIBUTING.md`, source-code comments anywhere in `src/`, and predecessor `specs/0XX-*/spec.md` files. Stale tool-name references in those un-rewritten files age along with the rest of their content; future BIs that touch those files for other reasons MAY incidentally update tool-name references they encounter, but this BI does NOT proactively sweep them.
- **FR-021**: The renamed tool implementations' factory function names MUST be updated to match the new tool names (per Clarifications Q1, 2026-05-12): `createReadNoteTool` → `createReadTool`, `createDeleteNoteTool` → `createDeleteTool`, `createListFilesTool` → `createFilesTool`, `createWritePropertyTool` → `createSetPropertyTool`, and `createRenameNoteTool` → `createRenameTool`. This is an internal-code rename and has no caller-visible effect; it is mandatory because retaining factory names that name a retired tool would create drift between the registry and the source.
...
- **Changes to predecessor `specs/0XX-*/spec.md` files, `CLAUDE.md`'s retained-narrative blocks, `.decisions/` ADR text, `.architecture/` docs, `CONTRIBUTING.md`, and source-code comments.** Per Clarifications Q3 (2026-05-12), the rename sweep is scoped narrowly to `README.md`, `docs/tools/*.md`, and `CLAUDE.md`'s active-narrative block. The remaining files are not proactively rewritten — stale tool-name references age along with the rest of their content and may be updated incidentally by future BIs that touch them for other reasons.
