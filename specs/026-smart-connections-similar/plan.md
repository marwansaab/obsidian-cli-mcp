...
- **BI-022 registry-stability baseline roll-forward**: the new tool's fingerprint added to [src/tools/_register-baseline.json](../../src/tools/_register-baseline.json) via `npm run baseline:write` post-implementation.
...
**Coverage gate**: aggregate statements floor is 91.3% ([vitest.config.ts:20](../../vitest.config.ts#L20)); the new tests must not drop the aggregate below this. The new module is ~230 LOC; the 63 co-located test cases (per the /speckit-analyze C1 remediation — handler count is 38, was stated as 32 in initial /speckit-tasks output) provide near-100% coverage of the new module, so the aggregate either stays flat or ratchets up.
...
├── data-model.md                                    # Phase 1 — input/output/eval-envelope schema shapes, JS template, base64 payload, per-tool invariants, module LOC budget, test inventory (63 cases per /speckit-analyze C1 remediation), architectural delta map
...
- **[data-model.md](./data-model.md)** — input/output/eval-envelope schema shapes, JS template, base64 payload assembly, per-tool invariants table, module LOC budget, test inventory (20 / 38 / 5 = 63 cases per /speckit-analyze C1 remediation; was 20 / 32 / 5 = 57 in initial output), architectural delta map vs predecessors.
...
| II. Public Surface Test Coverage | ✅ PASS | Test-set inventory frozen at 63 cases (20 schema / 38 handler / 5 registration — handler count is 38 per the /speckit-analyze C1 remediation; was stated as 32 in initial output, but the per-group breakdown summed to 38 and the 6 FR-017b precedence-chain fixtures are now DEDICATED cases rather than displacing existing error-path slots). Drift detector + FR-018 baseline both auto-cover. Each handler test responds to ONE spawn invocation per call per R3 (or 2 calls for the stage-0 closed-vault tests — main eval + vaults lookup). Base64 round-trip assertion in every payload-affecting handler test locks R6. Six dedicated compound-failure precedence-chain fixtures lock FR-017b per SC-011b. |
...
- **Generated artifacts**:
  ...
  - `specs/026-smart-connections-similar/data-model.md` — schema shapes, JS template, base64 payload, test inventory (63 cases per /speckit-analyze C1 remediation), architectural delta map
