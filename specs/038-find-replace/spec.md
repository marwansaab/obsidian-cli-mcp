...
- **Find request**: The pattern, the pattern mode (`literal` or `regex`), the replacement text, the optional case-sensitivity control, the optional subfolder scope, the include-code-blocks and include-html-comments opt-ins, and the commit opt-in. Defines what the agent asked for.
...
- **SC-006**: Unknown subfolder, invalid regex (in `regex` mode), safe-upper-bound-exceeded, and commit-time occurrence-count drift are surfaced as four distinct typed-error discriminators so an agent can branch on the error sub-code without parsing human-readable error prose. Drift surfaces under the existing `VALIDATION_ERROR` top-level code with `details.code: "OCCURRENCE_COUNT_DRIFT"` (ADR-015 sub-discriminator pattern), preserving the project's eleven-tool zero-new-top-level-codes streak.
