...
- **FR-005**: The system MUST preserve Windows happy-path behaviour byte-for-byte from the v0.3.0 baseline when running on Windows: the resolver attempts `OBSIDIAN_BIN` first, then defers to `PATH` lookup via the OS spawn. No platform-default file-existence pre-check fires on Windows. The structured-error `details` field gains the same enriched shape on Windows as on macOS and Linux per FR-004 — this is NOT a Windows-behaviour change in the contractual sense because the legacy `details: { binaryAttempted, PATH }` shape was internal diagnostic content, not part of any agent-visible contract (see R6 in research.md). Successful tool calls and the `code` of failed tool calls remain identical to v0.3.0.
...
- **FR-008**: When `OBSIDIAN_BIN` is set, the resolver MUST treat its value as a single, exact-path attempt — neither prefix-matched, glob-expanded, nor `PATH`-resolved. The override is the agent-operator's explicit naming of the binary. If the override fails to resolve to an executable file (whether because the file does not exist, the file exists but is not executable, or any other access-time error), the resolver MUST throw the structured `CLI_BINARY_NOT_FOUND` error with the override attempt's outcome label populated, and MUST NOT fall through to the platform-default or `PATH` branches. FR-020 below captures the specific worked example of "exists but not executable"; both ENOENT and non-ENOENT access failures are governed by this clause.
...
- **FR-020** (worked example of FR-008): When `OBSIDIAN_BIN` is set to a path that exists but for which `fs.access(path, fs.constants.X_OK)` fails (typically EACCES, EPERM, or any non-ENOENT errno), resolution MUST fail with the structured `CLI_BINARY_NOT_FOUND` error and the override attempt's outcome label MUST be `"found-but-not-executable"` (distinguishing it from the ENOENT-driven `"not-found"` label). The override is the user's explicit naming, and a permission failure on the named binary is a user-fixable misconfiguration that should be surfaced loudly, not masked by silent fallback. This is the specific case of FR-008's general no-fall-through rule.
