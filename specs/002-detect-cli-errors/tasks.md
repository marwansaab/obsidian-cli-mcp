...
- [ ] T001 Verify baseline at HEAD: run `npm run lint && npm run typecheck && npm run build && npm test` and confirm all four pass and coverage is at or above the FR-012 floor (84.3% statements). Capture the baseline coverage number to compare against the post-implementation number in T015.
