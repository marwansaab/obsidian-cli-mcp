...
**Created**: 2026-05-10
**Status**: Ready for implementation (post-`/speckit-analyze` 2026-05-10; rewritten 2026-05-10 after the original eval-bypass premise was retracted — see [bug-report-draft.md](bug-report-draft.md) and [ADR-009](../../.decisions/ADR-009%20-%20Direct%20Filesystem%20Write%20Path%20Alongside%20CLI%20Bridge.md) for the design pivot)
...
### Analyze Remediation 2026-05-10

`/speckit-analyze` 2026-05-10 surfaced one CRITICAL finding (C1) plus several HIGH/MEDIUM/LOW findings. All remediated in-place; the bullets below capture the resolution per finding ID:

- **C1 — Logger interface mismatch**: spec FR-029 originally called `logger.warn({event: "pathEscapeAttempt", ...})`, but the project's `Logger` interface at `src/logger.ts:43-48` exposes only typed per-event-type methods (`shutdown`, `dispatchTimeout`, `dispatchCap`, `dispatchKill`) — no generic `warn` / `info` / `debug` method. Resolution: extend `Logger` with a typed `pathEscapeAttempt(event: PathEscapeAttemptEvent)` method following the existing per-event-type convention. FR-029 rewritten to call the typed method. Best-effort `metadataCacheInvalidationFailed` and `openInEditorFailed` debug events (originally `logger.debug` calls in the contracts) are dropped — silent best-effort per Constitution IV's authorised carve-out cited by FR-011 + FR-017. New foundational task T002a added to tasks.md to extend the Logger interface before any handler tasks consume it.
- **H1 — FR-011 failure-or-boundary coverage gap**: data-model.md handler test cases #9 (rename-failure tmp cleanup), #10 (UUID uniqueness for concurrent tmp), and #12 (cache invalidation eval failure → success envelope) were not assigned to any task. Constitution Principle II requires each public surface ship with at least one happy-path AND one failure-or-boundary test in the same change; FR-011's failure-or-boundary half (case #12) was the load-bearing gap. Resolution: tasks.md T011 case range expanded to include #7, #9, #10, #11, #12, #30.
- **M1 — vault=Unknown handler-layer coverage**: data-model.md case #7 not explicitly assigned. Resolution: included in T011's expanded case range above.
- **M2 — SC-008 mid-write SIGTERM atomicity**: no CI-gated test (deterministic SIGTERM tests are unreliable in vitest). Acknowledged as a known gap; manual M-4 in [quickstart.md](quickstart.md) is the gate. Resolution: tasks.md T031's note documents the gap explicitly.
- **M3 — T011 case-range mismatch**: prose mentioned case #30 but numeric range topped at #28. Resolution: subsumed by H1's expanded range.
- **M4 — `ErrorCode` union not amended**: `src/logger.ts:4-10` `ErrorCode` type union doesn't include the three new codes (`PATH_ESCAPES_VAULT`, `FILE_EXISTS`, `FS_WRITE_FAILED`). Resolution: foldd into the new T002a task — Logger extension and ErrorCode amendment land together.
- **L1 — spec status "Draft"**: flipped to "Ready for implementation (post-`/speckit-analyze` 2026-05-10)" in the spec header.
- **L2 — `created`-flag benign TOCTOU not in spec edge cases**: documented in the new edge case bullet below.
...
- **Benign `created`-flag race in overwrite=true mode**: the handler determines `created` (true if path was absent before the write, false if it was present) by checking existence before the temp-write step. Between that check and the rename, a concurrent process could create or delete the target. The rename still succeeds with overwrite=true semantics regardless, so the on-disk content is correct; only the `created` boolean in the response can be off by a microsecond's worth of vault state. Acceptable trade-off; documented per Analyze L2.
...
- **FR-029**: Whenever the runtime path-safety check (FR-014) rejects an input as `PATH_ESCAPES_VAULT`, the handler MUST emit a typed security event via the project's `Logger` interface — providing an operator-side audit trail for attempted bridge attacks. The event is emitted via a new typed method `logger.pathEscapeAttempt(event: PathEscapeAttemptEvent)` added to the existing `Logger` interface at `src/logger.ts`, following the project's existing per-event-type method convention (cf. `dispatchTimeout` / `dispatchCap` / `dispatchKill`). The `PathEscapeAttemptEvent` payload carries `{ vault: string | null, attemptedPath: string }` (`vault` is null in active mode where the focused vault is implicit). Other failure modes (`FILE_EXISTS`, `FS_WRITE_FAILED`, `VALIDATION_ERROR` for path shape) MUST NOT emit per-call logger events at the tool layer; they propagate through `registerTool`'s existing UpstreamError → tool-error envelope plumbing per the prior typed-tool precedent (011 / 012 / 013 / 014 / 015 — "thin handler; no per-call events"). The best-effort failure paths in FR-011 (cache invalidation eval failure) and FR-017 (post-write `open` eval failure) likewise emit no logger events — they are silent best-effort per Constitution IV's authorised carve-out (the spec FR-011 / FR-017 + Edge Cases bullet are the citations). The cli-adapter's existing `dispatchTimeout` / `dispatchCap` / `dispatchKill` events continue to fire for the new tool's small `eval` calls (vault-registry probe, focused-file probe, `metadataCache` invalidation, optional editor-open) per ADR-007. The `Logger` interface extension and the corresponding `ErrorCode` union amendment (adding `PATH_ESCAPES_VAULT`, `FILE_EXISTS`, `FS_WRITE_FAILED` per FR-020) land together as the foundational task `T002a` in tasks.md.
