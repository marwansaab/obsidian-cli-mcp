...
- Q: What is the replacement tool name for `tree`? → A: `paths` (mirrors the output field; matches the project precedent where the output noun is the tool name — `properties` returns `{count, properties}`, `links` returns `{count, links}`, so `paths` returns `{count, paths}`).
- Q: How is the `file`/`path` schema leak fixed? → A: Surgical inline `targetModeBaseSchema.omit({ file: true, path: true })` in `paths/schema.ts` only. The refinement helper `applyTargetModeRefinementForFolderScoped` stays unchanged. The sibling `files` tool's schema is untouched — its existing refinement-layer rejection continues to govern at runtime.
- Q: What is the maximum character length for the registered description? → A: 512 chars. Roughly twice the post-BI-022 `read`/`rename` precedent (316/319 chars for 3-parameter tools), sized to accommodate `paths`'s six parameters (target_mode, vault, folder, depth, ext, total) while staying well inside every common MCP-client display threshold. Rejected alternatives: 320 chars (too tight for the 6-param surface), 1 024 chars (matches the `files`/`outline` mid-era precedent that the user's input did NOT call out as problematic but is still in the bloat-trend zone).
...
**Independent Test**: After the change, run a `tools/list` call against the server and inspect the description for the `paths` tool (the renamed replacement of `tree`). Confirm: (a) the description starts with a sentence naming the flat path-list output shape; (b) no token in the description matches `/\bFR-\d+\b/`, `/\bBI-\d+\b/`, `/\bADR-\d+\b/`, `/\b(fifteenth|sixteenth|seventeenth) typed-tool wrap\b/`, or `_eval-vault-closed-detection`; (c) the description ends with a literal `help` pointer; (d) the character count is below the agreed cap. No interaction with the file system or the vault is required.

**Acceptance Scenarios**:

1. **Given** the server is running and a client connects, **When** the client requests `tools/list`, **Then** the `paths` tool (the renamed replacement of `tree`)'s `description` field opens with a sentence that names the output shape `{ count, paths: string[] }` and characterises `paths` as a flat list before any other detail.
...
**Independent Test**: After the change, fetch the `paths` tool (the renamed replacement of `tree`)'s `inputSchema` from `tools/list`. Enumerate the top-level properties under `properties`. Confirm `file` and `path` are absent. Construct a call using only the fields that ARE in the schema with valid values (for example `{ target_mode: "specific", vault: "X", folder: "Y/" }`) and confirm the call does not fail with `VALIDATION_ERROR` citing an unknown field. No interaction with the file system or a real CLI is required if the schema-validation layer is exercised in isolation.

**Acceptance Scenarios**:

1. **Given** a `tools/list` response, **When** the `paths` tool (the renamed replacement of `tree`)'s `inputSchema.properties` keys are enumerated, **Then** `file` and `path` are NOT present.
...
- **Schema-fix implementation (RESOLVED at clarify stage)**: Surgical inline `targetModeBaseSchema.omit({ file: true, path: true })` in `paths/schema.ts`. The refinement helper `applyTargetModeRefinementForFolderScoped` stays unchanged. The sibling `files` tool's schema is untouched. Two alternatives were considered and rejected: (a) introducing a new `targetModeFolderScopedBaseSchema` in `target-mode.ts` — judged premature abstraction with one consumer; (b) propagating the fix into the sibling `files` schema — explicitly out of scope per the user's input.
- **No cross-tool regression risk under option A**: Because the refinement helper is unchanged, the sibling `files` tool keeps its current runtime rejection of `file`/`path` verbatim. SC-011 (sibling-tool no-regress lock) is satisfied by construction: `files` schema and behaviour are byte-stable across this BI.
...
- **FR-001**: The `paths` tool (the renamed replacement of `tree`)'s published `inputSchema.properties` MUST NOT include `file` or `path` at the top level.
...
- **FR-009**: The registered description MUST mention the trailing-slash file-vs-folder discriminator as a fact relevant to call construction (folder entries end with `/`; file entries do not), but MUST NOT cite the spec-branch identifier of the historical spec where that discriminator was first decided.
...
- **FR-011**: The registered description MUST be at most 512 characters in length, measured as the UTF-16 length of the JavaScript string literal. The cap was selected at clarify stage (2026-05-15 session) as roughly twice the post-BI-022 `read`/`rename` precedent (316/319 chars for 3-parameter tools), sized for `paths`'s six-parameter surface and well inside every common MCP-client display threshold.
...
- **FR-013**: The renamed replacement of `tree` MUST be registered under the name `paths`. The name was selected at clarify stage (2026-05-15 session) because it mirrors the output field `paths: string[]` and matches the project precedent where the tool name equals the output noun (`properties` returns `{count, properties}`, `links` returns `{count, links}`). The name MUST NOT be `tree` and MUST NOT contain the substrings `tree`, `hierarchy`, `nested`, `subtree`, `branches`, `nodes`, or `children` (negative-constraint kept as a regression guard).
...
- **FR-020**: The architecture document `.architecture/Obsidian CLI MCP - Architecture.md` MUST be rolled forward in this BI's commit to update each occurrence of the literal `tree` that names the current tool to `paths`. Historical / structural anchors that reference the tool at a point in time (e.g. "the fifteenth typed-tool wrap", "the tool added by BI-029") are preserved as historical references. Plan-stage decision (`research.md` R3) promoted the original MAY to MUST per the BI-026..BI-031 precedent of rolling forward the canonical architecture doc in the same commit that mutates the tool registry, preventing the doc-truth drift window.
...
- **SC-001**: The character length of the registered description for the `paths` tool (the renamed replacement of `tree`) is at most 512 (FR-011). Measured by reading `PATHS_DESCRIPTION` from the post-change registration module and counting the UTF-16 length of the literal.
...
- **SC-005**: The `paths` tool (the renamed replacement of `tree`)'s published `inputSchema.properties` keys are exactly the set `{target_mode, vault, folder, depth, ext, total}` (six keys; no `file`; no `path`).
...
- The replacement tool name is locked at clarify stage (2026-05-15 session) as `paths`. FR-013 carries the lock; the negative-substring constraint in FR-013 is retained as a regression guard rather than as a name-selection criterion. Runners-up considered and rejected at clarify: `find` (collision-adjacent with `find_by_property`), `walk` (less evocative of flat output), `list_paths` (re-introduces the `list_` prefix BI-022 retired).
- The 512-character description cap (FR-011) was locked at clarify stage (2026-05-15 session) after surveying sibling-tool description lengths: post-BI-022 renamed tools (`delete` 233, `move` 309, `read` 316, `rename` 319) demonstrate that tight descriptions are achievable; `paths` has roughly twice the parameter surface of `read`/`rename` (6 vs 3), so the cap is set at roughly twice the observed precedent. A tighter cap (320 chars, matching the precedent literally) is a stretch goal but not gating.
- The schema-fix implementation choice is locked at clarify stage (2026-05-15 session) as surgical inline `targetModeBaseSchema.omit({ file: true, path: true })` in `paths/schema.ts`. The refinement helper `applyTargetModeRefinementForFolderScoped` is untouched. The sibling `files` tool's schema and runtime behaviour are byte-stable. A future BI may extract a folder-scoped base schema when a second consumer materialises; rule-of-three not yet hit.
...
- The package version bump (currently `0.5.8`) lands as `0.6.0` (MINOR) per the project's pre-v1.0 release discipline and the BI-022 breaking-rename precedent (which bumped `0.4.4 → 0.5.0`, advancing the MINOR digit). The rename is a breaking surface change for any caller that has hard-coded the literal name `tree`, so MINOR is the correct granularity. Pinned at plan stage in `research.md` R2.
