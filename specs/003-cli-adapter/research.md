...
- **Binary path resolution**: `(deps?.env ?? process.env).OBSIDIAN_BIN ?? "obsidian"` per FR-006 (deps-aware form, updated from the original spec wording during /speckit-analyze remediation M1) and the [handler.ts:60-61](../../src/tools/obsidian_exec/handler.ts#L60-L61) precedent. Optional chaining handles the case where the entire `deps` argument is omitted by production callers; `?? process.env` handles the case where `deps` is supplied but `deps.env` is not.
