...
- Q: How should folder entries appear in the `paths` array when no `ext` filter is set (where the existing US1 / US3 examples implied a bare-name form but the spec did not explicitly lock the representation)? → A: **Trailing forward slash on folder entries; no suffix on file entries.** Folder entries MUST carry a trailing `/` (e.g. `Inbox/`, `Inbox/Sub/`, `Archive/`); file entries MUST NOT (e.g. `README.md`, `Inbox/a.md`, `Archive/old.md`). This provides in-band file/folder discrimination on every entry of the flat string array — callers do not need out-of-band knowledge (filename extension inspection, parallel calls, hierarchical-prefix scan) to tell which entries are folders. The wrapper-imposed lexical sort (FR-013) operates on the trailing-slash form. The discrimination rule is uniform: a path's terminal character (`/` or not-`/`) is the authoritative file-vs-folder signal — no fallback heuristic, no per-extension override, no version-drift surface. Codified by the new FR-028, locked into US1 / US2 / US3 / US4 example arrays, expanded into the FR-024 characterisation roster (a new bullet specifically asserts the trailing-slash representation on folder entries and its absence on file entries), and gated by the new SC-022.
...
1. **Given** vault `Demo` from US1 scenario 1, **When** the agent calls the tool with no `folder` (vault root implied) and `depth: 1`, **Then** the response `paths` contains only the immediate children of the vault root: `README.md`, `Inbox/`, `Archive/` (folders carry trailing slash per FR-028; the file `README.md` does not). The deeper entries `Inbox/a.md`, `Inbox/b.md`, `Inbox/Sub/`, `Inbox/Sub/c.md`, `Archive/old.md` MUST NOT appear.
2. **Given** the same fixture, **When** the agent calls the tool with `folder: "Inbox"` and `depth: 1`, **Then** the response `paths` contains only the immediate children of `Inbox/`: `Inbox/a.md`, `Inbox/b.md`, `Inbox/Sub/`. The deeper entry `Inbox/Sub/c.md` MUST NOT appear. The starting folder (which would render as `Inbox/`) itself does NOT appear.
...
1. **Given** vault `Demo` from US1 scenario 1, **When** the agent calls the tool with no `folder` and `ext: "md"`, **Then** the response `paths` contains `README.md`, `Inbox/a.md`, `Inbox/b.md`, `Inbox/Sub/c.md`, `Archive/old.md` AND does NOT contain any folder entry (no `Inbox/`, no `Inbox/Sub/`, no `Archive/`). Every entry in `paths` is a file (no trailing slash) per FR-028.
...
- A `folder` input with a trailing slash (`Inbox/`) versus without (`Inbox`) MUST yield the same response. If the underlying CLI does not natively treat them as equivalent, the handler normalises before passing through. INPUT-side trailing slash is normalised away; OUTPUT-side trailing slash is the FR-028 discrimination rule for folder entries in `paths`. The empty string (`folder: ""`) and bare forward slash (`folder: "/"`) are also normalised to the vault-root sentinel, IDENTICAL to `folder` omitted entirely — all three forms select the vault root as the starting folder.
...
- The OUTPUT-side representation of folder entries in `paths` is locked by FR-028: every folder entry ends with `/`, every file entry does not. This is independent of how the underlying CLI represents folder entries — the wrapper imposes the trailing-slash form. The input-side `folder` normalisation (trailing-slash-equivalence per FR-014) and the output-side `paths` representation (trailing-slash discrimination per FR-028) are distinct contracts; the trailing slash plays different roles on each side.
...
- **FR-024**: The feature MUST run a live-CLI characterisation pass before ship that documents observable CLI behaviour for each of the following cases. Findings MUST be persisted in the feature's research artefact.
  ...
  - Listing a subtree with mixed files and folders — confirms FR-028 trailing-slash invariant: every folder entry in `paths` ends with `/`, every file entry does NOT end with `/`. Asserted byte-exactly across the response.
...
- **FR-028**: Folder entries in `paths` MUST carry a trailing forward slash (`/`); file entries in `paths` MUST NOT. This rule is universal across every entry — files always render without a trailing slash; folders always render with one. The discrimination rule provides in-band file/folder typing on the flat string array — callers do not need out-of-band knowledge (extension lookup, hierarchical-prefix scan, parallel call) to distinguish file from folder. The terminal character of each path string (`/` or not-`/`) is the authoritative file-vs-folder signal — no fallback heuristic, no per-extension override, no version-drift surface. The wrapper-imposed lexical sort (FR-013) operates on the trailing-slash form (so the byte-asc ordering reflects the final published representation). When `ext` is set, no folder entries appear in `paths` at all (per FR-007) — so the trailing-slash rule is moot on that branch but still well-defined. When `ext` is omitted, both files and folders appear and the trailing-slash invariant is observable in tests. The starting folder is NEVER returned (per FR-012) regardless of whether it would render with a trailing slash. Path-traversal-rejected, missing-folder, and not-a-folder cases surface as structured errors (per FR-011 / FR-017) and the trailing-slash rule is moot on those paths.
...
- **Recursive listing response**: An object with two fields: `count` (a non-negative integer) and `paths` (an array of vault-relative path strings). On the `total: false` branch, `count === paths.length`. On the `total: true` branch, `paths === []` and `count` carries the full entry count after all filtering. Folder entries in `paths` end with `/`; file entries do not (per FR-028) — the trailing character of each path string is the in-band file-vs-folder signal. The response is the only success-path return value; any failure surfaces as a structured error, never as a partial `paths` array.
...
- **SC-022**: For a fixture subtree containing both files and folders, every folder entry in `paths` ends with `/` AND every file entry does NOT end with `/`, in 100% of test runs (per FR-028). The trailing-slash invariant is verified byte-exactly in regression tests. When `ext` is set, the `paths` array contains zero folder entries (and the trailing-slash rule is moot on that branch).
