...
| **ADR-015 (Sub-Discriminators via details.reason for Multi-State Error Codes)** | PASS (consumer) | BI-029 introduces TWO new `details.code` values (`FOLDER_NOT_FOUND`, `NOT_A_FOLDER`) under the existing `CLI_REPORTED_ERROR` top-level code, and inherits the BI-026 `VAULT_NOT_FOUND` with `details.reason: "unknown" \| "not-open"` sub-discriminator (via the shared closed-vault-detection module). ADR-015's worked-example pattern applies cleanly. Per constitutional explanation: "a PR that introduces no new (top-level-code, details.code) pair with multiple sub-states AND adds no new sub-states to existing pairs is N/A on ADR-015" — BI-029 DOES introduce two new (top-level-code, details.code) pairs, so the ADR-015 row is PASS not N/A. (Note: neither new pair has multi-state sub-discriminators internally, so the worked-example specifically does not multiply — but the ADR governs whenever ANY new details.code is added, per the constitutional row.) |
...
.architecture/Obsidian CLI MCP - Architecture.md  # MODIFIED — rolled forward to mention BI-029 as 4th _eval-vault-closed-detection/ consumer
```
