...
**Accepted wart**: `targetModeBaseSchema` declares `file`/`path` as optional, so the published JSON schema lists them even though the refinement always rejects them — identical to `files`. Cohort parity over a bespoke minimal schema. (If a reviewer prefers a clean published shape, a `{ target_mode, vault? }.strict()` base with a small custom refinement is the fallback; deferred unless requested.)
