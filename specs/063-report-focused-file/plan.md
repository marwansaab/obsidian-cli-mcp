...
- **Template** — a plain frozen `obsidian eval` IIFE that reads `app.workspace.getActiveFile()` and returns `{ active: f ? { path, name, basename, extension } : null }` (no `ok`-wrapper — `getActiveFile()` has no in-eval failure arm, so there is no `ok:false` case to discriminate; the body emits the output shape directly, and `decodeEvalEnvelope` validates it straight into `getActiveFileOutputSchema`). **No `composeEvalCode` payload** and no `__PAYLOAD_B64__` — the tool injects **no caller-supplied data** into the eval (active/specific routing is carried by `invokeCli`'s `vault`/`target_mode`, not the template), so there is no anti-injection surface (unlike `backlinks`, which interpolates `path`/`file`).
- **Handler** — `specific` mode: a pre-eval `resolveVaultRootOrRemap(vaultRegistry, input.vault, "get_active_file")` produces the typed `VAULT_NOT_FOUND/unknown` error for an unregistered vault (FR-010; the returned base path is discarded — same as `open_file`). Then `invokeCli({ command: "eval", vault: specific ? input.vault : undefined, parameters: { code }, flags: [], target_mode })`. Decode via the shared `decodeEvalEnvelope(stdout, getActiveFileOutputSchema, { toolName: "get_active_file", malformedCode: "CLI_REPORTED_ERROR" })` — the decoded value IS the result (carrying `null` straight through — **no `ERR_NO_ACTIVE_FILE`**, no re-parse step). Recovery (closed vault → ADR-029 cold-start retry; app down → ADR-030 launch) is inherited from `dispatchCli` and vault-correct because the call carries `vault=requested`.
...
│                     #       doubles as the eval-decode target (no separate ok-wrapped envelope schema)
├── schema.test.ts    # NEW — mode refinement (vault per mode; file/path rejected both modes; unknown field)
├── _template.ts      # NEW — frozen eval IIFE reading getActiveFile() → {active:{4 fields}|null} (no ok-wrapper);
