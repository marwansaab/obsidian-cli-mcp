...
- **Registration** — `server.ts` gains one `createGetActiveFileTool({ logger, queue, vaultRegistry })` line; `_register-baseline.json` gains a `get_active_file` entry (regenerated fingerprints). Tool docs `docs/tools/get_active_file.md` for `help`.
