...
- **Named vault is closed but registered**: verification determines, per tool, whether the read/query routes and recovers (as open_file does) or surfaces a structured "vault not reachable" signal (US3) — never a silent answer from the focused vault.
...
- **FR-008**: Documentation MUST keep a genuine residual limitation (e.g. a same-display-name collision) clearly distinct from the now-falsified blanket routing caveat, so a caller can tell what actually constrains cross-vault use and what does not.
- **FR-009**: After this feature, each cohort tool's published contract MUST resolve to exactly one of two confirmed states — "cross-vault routing confirmed (false caveat removed)" or "genuine limitation confirmed (real limitation stated, structured signal applied where applicable)" — with no tool left carrying the unverified blanket caveat.

#### Structured signal for a genuine limitation

- **FR-010**: A tool that genuinely cannot operate on a named non-focused vault MUST return a structured, typed error that identifies the unreachable vault, rather than silently returning a result computed from the focused vault.
...
- **FR-013**: Any structured signal introduced MUST reuse the project's existing error vocabulary — no new top-level error code and no new error sub-reason — per the additive-only closed-enum rule. If verification surfaces a genuinely distinct state that no existing code covers, that MUST be raised as its own separate item rather than minted here.
- **FR-014**: This feature MUST NOT add net-new tool behaviour beyond (a) correcting documentation and (b) adding a clear structured signal where a real limitation remains. If verification shows a tool needs genuine new cross-vault behaviour to route correctly, that MUST be captured as its own separate item, not delivered here.
...
- **Structured cross-vault signal**: A typed error that identifies the unreachable named vault, returned wherever a tool genuinely cannot serve a call from that vault — distinct from success and from the unknown-vault error.
...
- **SC-005**: For any tool that genuinely cannot reach a named non-focused vault, a structured error identifying that vault is returned in 100% of such calls, and a silent wrong-vault answer is returned in 0% of them.
...
- **No net-new routing behaviour is built here**: The feature verifies, documents, and adds a fail-loud signal where needed. If a tool turns out to need genuine new behaviour to route cross-vault, that is a separate item (FR-014), not part of this feature.
...
- Any net-new tool behaviour beyond correcting documentation and adding a clear structured signal where a real limitation remains. If verification shows a tool needs genuine new cross-vault behaviour, that is captured as its own separate item.
