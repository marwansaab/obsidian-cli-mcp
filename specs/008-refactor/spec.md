...
- Q: When `obsidian_exec` is invoked with `timeoutMs` exceeding the 120 s ceiling, how does the system enforce the ceiling? → A: Clamp silently to 120 s; the call proceeds with `timeoutMs = 120_000` and no error is raised.
- Q: How should the client-observable shift of `ERR_NO_ACTIVE_FILE` becoming newly reachable through `obsidian_exec` be handled? → A: Document the shift in CHANGELOG / release notes as a behavior refinement; no new global error codes are introduced; clients pattern-matching on `CLI_REPORTED_ERROR` for the no-active-file case must be alerted.
- Q: Should the dispatch primitive emit log lines for noteworthy lifecycle events? → A: Failure-only stderr logging — emit one stderr line each for timeout fired, output cap exceeded, and SIGINT/SIGTERM-driven child kill. No success-path logs. stdout reserved for MCP protocol.
- Q: When multiple typed tools have missing `docs/tools/{name}.md` files at boot, does the eager assertion fail-fast or aggregate? → A: Aggregate-and-report-all — the registration walk collects every missing doc and raises a single error listing all of them; one fix cycle resolves everything.
- Q: How is the SIGINT-vs-spawn-vs-registry-insertion race closed? → A: Mandate atomic insertion — registry insertion MUST occur synchronously after `spawn()` returns, before any `await` or microtask boundary in the dispatch flow.
...
- **FR-005**: The single tool-registration entry point MUST eagerly assert the existence of a `docs/tools/{name}.md` file at registration time. When one or more files are missing, server boot MUST fail with a single aggregated error that names **every** missing file — the registration walk completes its scan, collects every missing doc, and only then raises. Fail-fast on the first missing file is NOT acceptable (it forces N boot cycles for N missing docs and obscures the full picture from the developer). Lazy detection at help-tool runtime is also not acceptable.
...
- **FR-011**: A public escape-hatch surface — `invokeBoundedCli(input, overrides)` — MUST exist and apply default bounds (30 s timeout, 10 MiB output cap) overridable by the caller's `overrides` argument. Timeout overrides MUST be subject to a hard ceiling of 120 s; a caller-supplied `timeoutMs` exceeding the ceiling MUST be silently clamped to 120 s (the call proceeds with the clamped value, no `VALIDATION_ERROR` is raised, and no warning is emitted). Output-cap is not overridable today (matches today's behaviour). `obsidian_exec` MUST route through this surface; the agent-facing `timeoutMs` field on `obsidian_exec`'s MCP schema flows through to the `overrides` argument unchanged.
...
- **FR-015a**: Registry insertion MUST be atomic with `spawn()` — the inserted reference MUST be in place before any `await` or microtask boundary in the dispatch flow following `spawn()` return. This closes a narrow race where a SIGINT arriving between `spawn()` return and registry insertion would otherwise allow a live child to escape `killInFlightChildren`'s reach (the orphan that US3's central guarantee forbids). Symmetrically, registry removal at child exit MAY be asynchronous; only insertion is bound by the synchronicity rule.
...
- **FR-018a**: The dispatch primitive MUST emit a single-line stderr log entry for each of the following failure-lifecycle events: (i) a timeout fires (the `CLI_TIMEOUT` classification path), (ii) the output cap is exceeded (the `CLI_OUTPUT_TOO_LARGE` classification path), (iii) the in-flight child is killed by `killInFlightChildren` during shutdown (SIGINT / SIGTERM-driven). Each line MUST be written to stderr — never stdout, which is reserved for the MCP protocol — and MUST carry sufficient context for forensic reconstruction (at minimum the event type, a timestamp, and an identifier locating the dispatch such as the tool name and child PID). Success-path classification verdicts (`CLI_NON_ZERO_EXIT`, `ERR_NO_ACTIVE_FILE`, `CLI_REPORTED_ERROR`, `CLI_BINARY_NOT_FOUND`, success) MUST NOT emit log lines from the dispatch primitive — those surface to the caller through the response envelope only. The exact log format (e.g., JSON vs. key-value plaintext) is a plan-stage decision; the contract here is failure-only, stderr-only, single-line, with reconstruction context.
...
- **FR-021**: No new error code identifiers are introduced into `src/errors.ts`. The unified classification path does, however, make `ERR_NO_ACTIVE_FILE` (already present in the `read_note` roster per feature 003's contract) newly reachable through `obsidian_exec` for inputs that produce `Error: no active file` on stdout — where today the same input surfaces as `CLI_REPORTED_ERROR`. This client-observable shift MUST be called out in the release CHANGELOG / release notes as a behavior refinement so clients pattern-matching on `CLI_REPORTED_ERROR` for the no-active-file case are alerted, and `docs/tools/obsidian_exec.md` MUST be updated to list `ERR_NO_ACTIVE_FILE` among the codes the tool can return.
...
- **SC-011**: The dispatch primitive emits exactly **one** stderr log line for each of the three failure-lifecycle events (timeout fire, output cap exceeded, SIGINT/SIGTERM-driven kill) and **zero** log lines on the success path or other classification verdicts. *(Measured by: automated tests that capture stderr during synthetic failure-path runs and assert line-count plus structural content, plus a no-log assertion on a successful spawn.)*
