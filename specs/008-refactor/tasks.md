...
- [ ] T026 [US3] Update [src/server.test.ts](../../src/server.test.ts) to verify the new import path (e.g., a static check or `import.meta`-introspection equivalent) and to drive the shutdown handler end-to-end: with a mid-flight in-flight child → `triggerShutdown` reports `inFlightKilled: true`; with no in-flight child → reports `inFlightKilled: false`; shutdown is idempotent under double-invocation (FR-018 — semantics unchanged). Depends on T025.
...
- [ ] T029 [P] [US4] Grep verification (manual or scripted): zero direct calls to `zodToJsonSchema`, `stripSchemaDescriptions`, or `toMcpInputSchema` outside [src/tools/_register.ts](../../src/tools/_register.ts), [src/tools/_shared.ts](../../src/tools/_shared.ts), and their `*.test.ts` files (SC-002 binding). Document the verification commands and outputs in this task's commit message (or in a temporary note that gets discarded after the polish phase).
...
**Final Checkpoint**: Feature ready to merge. Tag `v0.2.0` and run release pipeline (SC-010) per quickstart §SC-010.
...
- T036 (quickstart validation) depends on T015–T035.
