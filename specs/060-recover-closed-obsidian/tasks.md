...
- [ ] T003 [P] Extend the logger in `src/logger.ts`: add `DispatchRecoveryEvent` (data-model §5) and a `dispatchRecovery(event)` method on the `Logger` interface and `createLogger` (emit a `dispatch.recovery` JSON-line). Do NOT add any new entry to the `ErrorCode` union (Principle IV — zero new top-level codes). **Add a `dispatch.recovery` emit test to `src/logger.test.ts`** asserting the JSON-line shape for each `outcome` (`recovered`/`unrecoverable`/`disabled`), at parity with the existing event-emit tests (analyze C2).
