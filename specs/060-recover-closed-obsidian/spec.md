...
- **Auto-recovery is on by default with an environment opt-out** (Clarification 2026-05-30): recovery runs by default; the environment variable `OBSIDIAN_AUTO_LAUNCH=0` (fixed in plan — research D5) disables it, following the existing `OBSIDIAN_BIN` env-config precedent in `binary-resolver`. With the opt-out set, no launch is attempted and the FR-007 distinct error is surfaced. A richer configuration surface beyond this single on/off opt-out, headless operation, and window hiding all remain out of scope.
...
### Resolved at plan-phase (T0 live-CLI probe, 2026-05-30 — see [research.md](research.md) and [contracts/t0-probe-findings.md](contracts/t0-probe-findings.md))

These timing/signature parameters were deferred at spec time and are now pinned in the plan (the spec fixes the behaviour; the plan fixed the numbers and literals):

- **Application-not-running signal**: `CLI_NON_ZERO_EXIT` (exit 1, empty stdout) whose stderr matches `/unable to find Obsidian/i` — distinct from the ADR-029 cold-start `exit 0` + `Command "<cmd>" not found.` and from unrelated CLI failures (research D1).
- **"Ready" + bound**: readiness is observed by re-dispatching the original command; bounded poll at ~750 ms up to 30 000 ms (T0: ~3 s on Windows) (research D3).
- **Distinct-error literal**: reuse `CLI_NON_ZERO_EXIT` + `details.reason = "obsidian-not-running"` (encoding mechanism decided per Clarification 2026-05-30; literal pinned in research D4). **Env opt-out variable**: `OBSIDIAN_AUTO_LAUNCH` (research D5).
