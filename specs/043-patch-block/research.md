...
**Decision**: The block-id scanner walks the note line-by-line, tracking three pieces of state — `inFence: boolean`, the previous line's content (for setext-heading detection), and a per-line `^block-id` match. When a `^block-id` token is detected, the scanner classifies its enclosing block shape using these rules:
...
| R2  | Scanner classifies five shapes (paragraph / list-item / separately-placed / on-heading-atx / on-heading-setext) with single-line setext lookahead | FR-008 / FR-009 / FR-010 / FR-019a, cohort divergence from BI-040 R2 |
