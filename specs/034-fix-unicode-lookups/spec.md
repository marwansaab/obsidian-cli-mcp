...
- **FR-006**: Every operation in the cohort identified at plan stage (per Clarifications C-001) MUST preserve its existing behaviour for inputs whose characters all fall within the basic ASCII range — every input that succeeds today continues to succeed after this change, and every input that legitimately resolves to "not found" / empty today continues to do so.
- **FR-007**: Every operation in the cohort identified at plan stage (per Clarifications C-001) MUST continue to emit its existing response shapes, error envelopes, and error codes for both success and failure cases — this change does not introduce, remove, or rename any response field, sentinel value, or error code.
- **FR-008**: The fix MUST cover **at least** the three named operations (`read_heading`, `read_property`, `find_by_property`) **and additionally every other operation that, after the plan-stage audit, is found to share the same input-decode defect class as `read_heading` and `find_by_property`**. The cohort identified at plan stage and its rationale are recorded in Clarifications C-001. Operations that are not affected today (`read`, `write_note`, `delete`, `obsidian_exec`, `help`, `files`, plus the audit-verified-unaffected operations `outline`, `properties`, `rename`, `move`, `search`, `set_property`) continue to behave as they do today.
...
- **SC-004**: 0% regression on ASCII-only inputs to any operation in the cohort identified at plan stage (per Clarifications C-001) — every call that succeeds today continues to succeed after this change, and every call that legitimately returns "not found" / empty today returns the same response after this change.
- **SC-005**: 0 new fields, sentinels, or error codes appear in the published response shapes of any operation in the cohort identified at plan stage (per Clarifications C-001). The defect is invisible to callers who already exercise ASCII-only inputs; the fix is invisible to callers who exercise the response shape rather than the response content.
- **SC-006**: Every operation outside the cohort identified at plan stage (per Clarifications C-001) — including but not limited to `read`, `write_note`, `delete`, `obsidian_exec`, `help`, `files`, `outline`, `properties`, `rename`, `move`, `search`, `set_property` — shows 0% behavioural change. Their response shapes, success cases, and failure cases continue to match their existing test coverage. `read_property` is in the originally-named-three set but the plan-stage audit re-classified it as not sharing the defect class, so it also shows 0% behavioural change at the production-code layer and ships a verification test only (see Clarifications C-002).

## Clarifications

These clarifications were settled at plan stage (`/speckit-plan` on 2026-05-17) after a static audit of every tool's input-decode path. They are recorded here so the contract reflects the audit findings rather than the pre-audit framing alone. The audit's implementation-level detail (the specific decode primitive, transport, and per-tool source locations) lives in [research.md](research.md); this block carries only the contract-level conclusions.

- **C-001 (Scope of fix — cohort identified at plan stage)**: The plan-stage audit surveyed every tool's input-decode path and identified the cohort of operations that share the same defect class as `read_heading` and `find_by_property`. The fix covers all members of that cohort. Seven operations are in scope: `read_heading`, `find_by_property` (the two spec-named affected tools whose audit verdict matches the spec premise), plus `paths`, `links`, `tag`, `smart_connections_similar`, and `smart_connections_query` (the five additional tools that the audit found to share the same defect class, though they were not named in the originally-authored spec). Operations outside this cohort are confirmed unaffected by the same audit and continue to behave as they do today (see SC-006 for the enumerated unaffected set). Full rationale and per-tool verdict table: [research.md §1, §3](research.md).
- **C-002 (`read_property` audit verdict — predicted-passing, no production change)**: The plan-stage audit found that `read_property`, though named in the original three-tool list, does NOT share the defect class with `read_heading` and `find_by_property`. By static analysis it is predicted to already satisfy FR-003 with no production code change. User Story 2 still earns a verification test ([tasks.md T013](tasks.md)) so future regression is caught; the test is predicted-passing pre-fix. If the test fails unexpectedly, the BI escalates per [research.md §2.3](research.md) and a different defect path is investigated. The spec's FR-003 contract is satisfied either way — only the explanation of *why* moved from "shares the input-decode fix" to "static-analysis verified plus regression-test guarded".
- **C-003 (Test scope for the additional cohort members)**: The five tools added to scope by C-001 each receive a non-ASCII boundary unit test in the same change set per Constitution Principle II. Live-CLI T0 probes cover `paths`, `links`, `tag` ([quickstart.md](quickstart.md) Probes 4-6); `smart_connections_similar` and `smart_connections_query` are deliberately SKIPPED at the live-probe layer per [plan.md Complexity Tracking](plan.md) (bare-vault invariant in `.memory/test-execution-instructions.md` forbids installing the underlying plugin) and rely on unit tests for verification.

## Assumptions

- The defect is in the input-decode step shared by the cohort identified at plan stage (per Clarifications C-001), not in their per-handler comparison logic. The user's framing ("the identifier is corrupted in transit before the underlying comparison runs") locates the fault at the boundary, not in the matchers themselves, and explicitly scopes the fix to "the input-decode step." The cohort's handlers receive their string inputs through a different intake path than the unaffected operations — a difference identified as the reason `read`, `write_note`, etc. round-trip non-ASCII characters correctly today. `read_property` is named in the user stories but the audit re-classifies it as not sharing the defect class (see C-002).
...
- The cohort handlers continue to use their existing comparators. The fix is at the input boundary; per-handler matching semantics — and therefore each handler's existing success and failure tests — remain valid as the regression net for ASCII-only inputs.
