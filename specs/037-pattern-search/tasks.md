...
**Plan-stage findings carried forward** (re-stated here so implementers see them before writing code; full detail in [research.md](research.md)). Note: R11 is intentionally absent from this list because it is reified as Phase 2 (T001's six T0 sub-probes T0.1..T0.6) rather than as a carried-forward design decision.
...
- [X] T001 Live-CLI characterisation of T0.1..T0.6 against the authorised test vault per [.memory/test-execution-instructions.md](../../.memory/test-execution-instructions.md) (gated by CLAUDE.md `## Test Execution`). Capture stdout / stderr / exit code; append results to [research.md](research.md) under a new `## T0 Live-CLI Capture (yyyy-mm-dd)` section. Cleanup all fixtures from the scratch subdirectory after capture. Cases:
...
- [X] T006 [US1] Create [src/tools/pattern_search/handler.test.ts](../../src/tools/pattern_search/handler.test.ts) with ~34 cases. Inject stub `invokeCli` via `deps.spawnFn` stub returning `{stdout: '=> ' + JSON.stringify(envelopeFixture), stderr: '', exitCode: 0}` per the cli-adapter test-seam convention (mirror BI-036 / BI-035 pattern). Per-test assertions: (a) `spawnFn.mock.calls.length === 1` (single-spawn invariant per R2); (b) for any test exercising the payload, decode the `code=` argv via base64 → assert the decoded payload matches the user input bit-for-bit (R12 anti-injection structural lock); (c) for envelope-error tests, assert the thrown UpstreamError carries the correct `code` AND `details.stage` AND `details.code` AND (where applicable) `details.folder`.

  **Cross-layer note** (added per /speckit-analyze remediation 2026-05-17 D1): cases below have parity counterparts in [quickstart.md](quickstart.md) scenarios — T006 cases exercise the wrapper layer with stubbed CLI responses; quickstart scenarios exercise the full stack against the live test vault. Both layers are part of the test inventory: T006 is the regression net (runs on every CI), quickstart is the live-CLI characterisation gate (runs at /speckit-implement T0). Overlap is intentional. Map: T006 case 1 ↔ quickstart S1; T006 case 6 ↔ quickstart S6; T006 case 18 ↔ quickstart S3; T006 case 21 ↔ quickstart S4; T006 case 3 ↔ quickstart S7; T006 cases 10-12 ↔ quickstart S8.
  ...
  **Spec edge-cases + explicit structural locks** (added per /speckit-analyze remediation 2026-05-17 C1-C7):

  - (29) **FR-013 plain-text scanning — match inside fenced code block returned** (C1): stub envelope where a match's `path` points at a note known to contain the match inside a triple-backtick code block (e.g., `path: "Docs.md", line: 7, offset: 4, match: "TODO", text: "    TODO inside code fence"`). Assert the wrapper response includes the entry verbatim — the wrapper performs no markdown-aware exclusion. Locks the FR-013 contract at the wrapper layer; the template owns the actual "no markdown awareness" structural lock.
  - (30) **Match spans entire line** (C2): stub envelope `{ ok: true, count: 1, matches: [{ path: "Short.md", line: 1, offset: 0, match: "exact-full-line", text: "exact-full-line" }] }`. Assert `match === text` AND both fields are populated AND neither is truncated. Covers the spec Edge Cases bullet "the matched substring spans the entire line".
  - (31) **FR-012 line-scoped guarantee** (C3): stub envelope simulating the template's per-line split — input pattern `"foo\\nbar"` (newline metacharacter in source) against a note containing `"foo\nbar"` on two consecutive lines. The stub envelope returns `{ ok: true, count: 0, matches: [] }` (the template's `/\r?\n/` line-split removes the newline before regex matching, so the cross-line match cannot fire). Assert response is zero-match success, NOT an error. Locks FR-012 at the wrapper layer.
  - (32) **Valid folder, empty result** (C4): stub envelope `{ ok: true, count: 0, matches: [] }` returned from a payload carrying `folder: "EmptyDir"`. Assert response is empty success AND `truncated` is absent AND decoded payload carries `folder: "EmptyDir"`. Distinguishes valid-folder-empty from folder-not-found (case 21) at the wrapper layer. Covers spec Edge Cases bullet "folder scope is valid but the named folder is empty".
  - (33) **FR-007 case-insensitive behavioural pass-through** (C5 strengthening of case 24): stub envelope with case-mixed entries `[{ path: "P.md", line: 1, offset: 0, match: "TODO", text: "TODO upper" }, { path: "P.md", line: 2, offset: 0, match: "todo", text: "todo lower" }]` returned from a payload carrying `case_sensitive: false`. Assert the wrapper passes both entries through unchanged — no wrapper-side filtering by case. The actual case-folding behaviour is owned by the template's `i`-flag application (asserted at T0.1 / quickstart S3); this case locks the wrapper-layer transparency.
  - (34) **FR-015 read-only structural lock** (C7): static-source assertion. Import the `JS_TEMPLATE` string from `./_template.js` and assert it contains NONE of the mutating Obsidian API substrings — `app.vault.modify` / `app.vault.create` / `app.vault.delete` / `app.vault.adapter.write` / `app.vault.adapter.remove` / `app.fileManager.renameFile`. Paranoid but explicit; complements the structural lock that the template only invokes `getMarkdownFiles` / `cachedRead` / `adapter.stat`.

  Carry `// Original — no upstream. pattern_search handler tests.` header. Total: ~34 cases (28 original + 6 remediation cases 29-34).

- [X] T007 [US1] Create [src/tools/pattern_search/index.ts](../../src/tools/pattern_search/index.ts). Export `createPatternSearchTool(deps: RegisterDeps): RegisteredTool` via the `registerTool` factory (parity with `context_search` / `backlinks`). Export `PATTERN_SEARCH_TOOL_NAME = "pattern_search"` constant. Export `PATTERN_SEARCH_DESCRIPTION` string — a one-paragraph description covering: typed envelope shape `{ count, matches: [{path, line, offset, match, text}], truncated? }`; required `pattern` (ECMAScript dialect — `\d`, `\b`, lookahead, lookbehind, named captures, `i` flag per Q1 clarification; 1..1000 chars; invalid regex → VALIDATION_ERROR path:["pattern"]); optional `folder` (recursive subtree-prefix match, leading/trailing `/` stripped); optional `limit` (1..10000, implicit 1000); **CRITICAL — optional `case_sensitive` (default `true`, FLIPS from sibling `context_search` default `false` per spec FR-007)**; optional `vault` (routes to focused vault when omitted); response invariant `count === matches.length`; sort order `(path, line, offset)` ascending; `text` capped at 500 UTF-16 + `…` per Q2; `match` substring NEVER capped; zero-length matches skipped per Q3 (`^`, `$`, `a*`, `\b`, lookarounds emit no entries for their zero-width hits); `truncated: true` only when cap fired (absent ≡ false); zero-match valid-pattern returns empty success (NOT error per FR-009); folder-not-found surfaces as `CLI_REPORTED_ERROR(details.code: "FOLDER_NOT_FOUND")`; unknown vault surfaces via cli-adapter `Vault not found.` classifier; closed-but-registered vault surfaces as `CLI_REPORTED_ERROR(details.code: "VAULT_NOT_FOUND", details.reason: "not-open")`. Cross-pointer to `help({ tool_name: "pattern_search" })`. Cross-pointer to sibling `context_search` (BI-035) — "for literal keyword matching with simpler payloads, use `context_search`; pattern_search adds regex semantics at the cost of the per-line `offset` field and the case-sensitivity default flip". Carry `// Original — no upstream. pattern_search tool registration via registerTool — ECMAScript-regex search primitive returning a typed { count, matches: [{path, line, offset, match, text}], truncated? } envelope; responseFormat: "json" (default) wraps the envelope for the MCP wire. Sixteenth typed-tool wrap.` header.
...
- [X] T013 [P] Modify [CHANGELOG.md](../../CHANGELOG.md) — add a release entry under the next unreleased version section. Format mirror BI-036 / BI-035 entries:
  ```
  ## [0.6.5] - 2026-MM-DD
...
- [X] T014 [P] Modify [package.json](../../package.json) — bump `version` from `0.6.4` to `0.6.5` (PATCH per BI-023 / BI-024 / BI-025 / BI-035 / BI-036 additive-surface precedent). Also update `description` — add `pattern_search` to the alphabetically-sorted tool list (between `paths` and `properties`). No other field touched.

- [X] T015 Run quality gates in this order. Each MUST pass before merge:
  ...
  6. Aggregate `statements` coverage threshold passes — `npm test` reports ≥ **91.3%** (the current `vitest.config.ts` `test.coverage.thresholds.statements` floor at /speckit-analyze remediation 2026-05-17; the floor is read from `vitest.config.ts` at run time and may have ratcheted upward by the time T015 executes). The new module adds ~200 production LOC + ~1000 test LOC (test count bumped from ~50 to ~57 after the analyse-stage 2026-05-17 remediation added six T006 cases + one T003 case); coverage should remain flat or ratchet up.

- [X] T016 Verify the live-CLI characterisation pass from T001 produced no TRIGGER findings that require spec / plan amendments. If TRIGGERs were surfaced AND deferred, document them as known-gaps in [research.md § T0 Live-CLI Capture](research.md) and either (a) fix the wrapper before merge if the user-facing contract is broken, or (b) open a follow-up BI if the deviation is outside the user-facing contract.
