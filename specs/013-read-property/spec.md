...
9. **Given** a note whose frontmatter block is malformed (for example missing the closing `---` fence, stray colons inside flow-mapping values, or unmatched brackets), **When** the agent reads any property, **Then** the response is `{ value: null, type: "unknown" }` — the wrapper does not distinguish the malformed-frontmatter case from the absent-property case. (Behaviour captured by the BI-041 T0 probe T005 and authorised under Constitution Principle IV's intentional-best-effort-continue clause per [specs/041-reconcile-cohort-doc-drift/plan.md](../041-reconcile-cohort-doc-drift/plan.md) §Complexity Tracking; retirement of the prior "structured error" promise tracked by [specs/042-close-audit-findings/spec.md](../042-close-audit-findings/spec.md) User Story 1.)
...
- **FR-012**: The tool MUST surface a structured error when the file's frontmatter block is malformed (e.g., missing closing fence). The wrapper MUST NOT silently coerce a malformed frontmatter to a successful read. **Plan-stage amendment** (per [research.md R7](research.md#r7--no-frontmatter-found-response-tool-layer-detection-fr-011--fr-012-spec-amendment)): live-CLI characterisation revealed Obsidian conflates malformed frontmatter with no-frontmatter — both produce identical `No frontmatter found.` stdout, indistinguishable to the wrapper. This requirement is therefore weakened in operational terms — both cases follow FR-011's `{value: null, type: "unknown"}` semantic. The spec wording is preserved per the project convention of not retroactively amending spec.md (see [research.md R12](research.md#r12--dont-amend-predecessor-specs-project-convention)); this footnote makes the divergence discoverable.
...
- ~~The user input is exhaustive for ship-gating decisions: no clarifications session is required (`/speckit-clarify` is not needed).~~ **Superseded 2026-05-09**: a clarifications session DID run on 2026-05-09 and produced two amendments (Q1 absent-vs-explicit-null discriminator mechanism; Q2 mapping-value handling — added FR-027, the CONTENT — mapping values Edge Cases bullet, and the FR-024 case roster expansion 13 → 15). The 18 acceptance criteria across [P1] / [P2] / [P3], the six adversarial categories (CONCURRENCY, CONTENT, UNDERLYING CLI, CLIENT-CLASS, SECURITY), and the explicit out-of-scope list defined a near-complete spec surface; the two clarifications closed the remaining gaps before plan stage.
