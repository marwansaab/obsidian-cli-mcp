...
- **FR-008**: The tool MUST apply ASCII lower-fold (`String.prototype.toLowerCase()`) to BOTH the input tag AND every stored tag value inside the eval JS template, producing case-insensitive matching consistent with Obsidian's tag-pane UX. Lower-fold is ASCII-only — Unicode case-folding is out of scope at v1. (Amendment driver: plan-stage live probe F2 / Q1's explicit conditional fired; see Clarifications "Plan-stage live-probe amendments" block.)
