...
- Tag carried via frontmatter `tags:` list and inline body `#tag` syntax — both contribute equally; one or the other is enough for the note to match.
- Case variation between query and stored tag (query `foo` vs stored `Foo`, or vice versa) — treated as equivalent (case-insensitive match).
...
- **FR-006**: The tool MUST include matches whether the tag was declared via body inline `#tag` syntax or via frontmatter `tags:` list entry — both sources contribute equally to a note's tag set.
...
- **FR-008**: The tool MUST treat the queried tag and stored tags as case-insensitive equivalents for matching purposes (matches Obsidian's documented tag-matching convention).
...
- **FR-011**: The tool MUST reject inputs that are empty, whitespace-only, OR contain empty hierarchical segments (`/foo`, `foo/`, `foo//bar`) with a structured validation error BEFORE any vault scan.
...
- **Tag query**: a non-empty string representing the tag the caller wants to look up. May be entered with or without a leading `#`; hierarchical (`/`-separated) segments allowed; case-insensitive against stored tags.
- **Tag match**: a vault note whose declared tag set contains the query as an exact tag OR any descendant tag prefixed by `<query>/`. Body inline `#tags` and frontmatter `tags:` entries both contribute to the note's tag set.
...
- **SC-009**: Tag input with case variation against stored tags (`Foo` query vs `foo` stored, or vice versa) produces identical matches.
- **SC-010**: A note tagged with the same tag in multiple places (body twice + frontmatter) appears exactly once in `paths`.
- **SC-011**: An invalid input (empty, whitespace-only, malformed hierarchical segments) is rejected before any vault scan, with a structured validation error.
...
- Case-insensitive matching is the default because Obsidian's documented tag-matching convention treats tag identifiers case-insensitively, even when stored case is preserved.
