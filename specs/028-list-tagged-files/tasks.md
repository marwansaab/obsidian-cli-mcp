...
- [X] T006 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-1 default-mode happy path (data-model handler rows 25-29 envelope shapes): mock `invokeCli` returns `{stdout: '=> {"ok":true,"mode":"default","count":2,"paths":["a.md","b.md"]}', stderr: "", exitCode: 0}`; assert handler returns `{count: 2, paths: ["a.md", "b.md"]}` and `invokeCli` called exactly once. Include four sub-cases enumerating the envelope-shape variants from the data-model.md inventory: (a) `count: 2`, (b) `count: 0` empty paths (note: overlaps with T007; keep here for completeness as envelope-shape characterisation), (c) `count: 1` single match, (d) large array (`count: 1000+`) verifying no truncation. Plus the mode-mismatch case: when input.total=false but envelope `mode: "count-only"`, assert envelope-parse error (the discriminated union safeParse catches it).
- [X] T007 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-4 zero-match: mock stdout `=> {"ok":true,"mode":"default","count":0,"paths":[]}`; assert returns `{count: 0, paths: []}` — never throws.
- [X] T008 [P] [US1] Add to `src/tools/tag/handler.test.ts` — single-spawn invariant Q-14: assert `invokeCli` spy called exactly once for any successful call.
- [X] T009 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-11 (a-e) validation-error-before-spawn: assert spy NEVER called for empty/whitespace/empty-segment/length>200/unknown-key inputs.
- [X] T010 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-15 stage-0 closed-vault: mock empty stdout exit-0, assert handler throws `CLI_REPORTED_ERROR(VAULT_NOT_FOUND, reason: "not-open")`.
- [X] T011 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-16 stage-2 json-parse failure: mock stdout `=> not-json`, assert throws `CLI_REPORTED_ERROR(stage: "json-parse")`.
- [X] T012 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-17 stage-3 envelope-parse failure: mock stdout `=> {"ok":true,"bogus":1}`, assert throws `CLI_REPORTED_ERROR(stage: "envelope-parse")`.
- [X] T013 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-18 stage-4 envelope-error branch: mock stdout `=> {"ok":false,"code":"CACHE_NOT_READY"}`, assert throws `CLI_REPORTED_ERROR(stage: "envelope-error", code: "CACHE_NOT_READY")`.
- [X] T014 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-19 / Q-41..Q-43 anti-injection: three adversarial inputs (`\"); evil(); (`, newlines, backticks); assert rendered `code` parameter is byte-stable except for substituted base64 region; round-trip base64 decode preserves input.
- [X] T015 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-20 vault flow-through: assert `invokeCli` args lack `vault` key when input.vault is undefined; assert `vault: "X"` flows through verbatim when input.vault === "X".
- [X] T016 [P] [US1] Add to `src/tools/tag/handler.test.ts` — Q-5 deterministic ordering (data-model handler rows 44-48 sort+dedup invariants): paired calls with identical inputs produce byte-identical responses (handler does NOT re-sort — JS template owns the ordering). Include five sub-cases: (a) already-sorted paths from JS template — handler passes through; (b) unsorted paths from JS template — handler does NOT re-sort (responsibility is the template's, not the handler's); (c) empty paths array natural return shape; (d) single-path natural return shape; (e) 1000+ paths — no truncation, full passthrough.

### Implementation for User Story 1

- [X] T017 [US1] Implement `tagHandler` body in `src/tools/tag/handler.ts`: validation (zod parse → throws ZodError on failure → SDK serialises VALIDATION_ERROR); base64 payload assembly (`{query, total: !!input.total}` → JSON.stringify → Buffer.toString("base64") → replace `__PAYLOAD_B64__` placeholder); single `invokeCli` call with `subcommand: "eval"`, `parameters.code: <rendered>`, optional `vault` flow-through; stage-0 closed-vault detector consumption; stages 1-5 multi-stage parse with `UpstreamError` propagation per the handler contract.
- [X] T018 [US1] Implement the FROZEN_TEMPLATE constant body in `src/tools/tag/handler.ts` — the ~40-LOC JS template from data-model.md verbatim. Verify byte-stability across calls (T014 anti-injection tests cover this structurally). Template must include: base64 payload decode via `atob` + `JSON.parse`; query normalisation `q = String(payload.query).toLowerCase()`; cache walk over `app.metadataCache.fileCache`; `.md`-only filter; per-path Set with body inline tags AND frontmatter tags both normalised (strip leading `#`, lower-fold); `isMatch` returning `tagLower === q || tagLower.startsWith(q + "/")`; collect matching paths; `out.sort()` byte-asc; envelope emit branched on `wantTotal`.
...
- [X] T019 [P] [US2] Add to `src/tools/tag/handler.test.ts` — Q-2 hierarchical subsumption: mock stdout from a JS-template-walk-simulation envelope where `paths` contains the parent, child, and grandchild paths; assert handler returns all three.
...
- [X] T021 [P] [US3] Add to `src/tools/tag/handler.test.ts` — Q-3 leaf-precision: mock stdout where `paths` contains only `foo/bar` and `foo/bar/baz` paths (NOT the parent-only `foo` path); assert handler returns exactly those two.
- [X] T022 [P] [US3] Add to `src/tools/tag/handler.test.ts` — Q-13 segment-boundary precision: mock stdout where `paths` is `[]` because the only candidate tag was `foobar` (which must NOT match query `foo`); assert handler returns `{count: 0, paths: []}`.
...
- [X] T024 [P] [US4] Add to `src/tools/tag/handler.test.ts` — Q-6 count-only happy path: mock stdout `=> {"ok":true,"mode":"count-only","total":5}`; input `{tag: "alpha", total: true}`; assert returns `5` (bare integer, no paths surfaced).
- [X] T025 [P] [US4] Add to `src/tools/tag/handler.test.ts` — count-only zero-match: mock stdout `=> {"ok":true,"mode":"count-only","total":0}`; input `{tag: "nonexistent", total: true}`; assert returns `0` (no error).
- [X] T026 [P] [US4] Add to `src/tools/tag/handler.test.ts` — Q-21 cross-mode count invariant: paired calls with same input differing only in `total`; assert count-only result === default-mode paths.length.
- [X] T027 [P] [US4] Add to `src/tools/tag/handler.test.ts` — mode mismatch: when input.total=true but envelope says `mode: "default"`, assert envelope-parse error (the discriminated union safeParse catches it). And vice versa.

### Implementation for User Story 4

- [X] T028 [US4] Verify `tagHandler` stage-5 dispatches correctly between `tagDefaultOutputSchema` and `tagCountOnlyOutputSchema` based on `envelope.mode`. The implementation is from T017; this task confirms the cross-mode dispatch matches the contract. T024-T027 are the regression suite.
...
- [X] T029 [P] Create `src/tools/tag/index.ts` — export `createTagTool(deps: RegisterDeps)` factory returning `{name: "tag", description, inputSchema, handler}`. Carry Original-no-upstream header.
- [X] T030 [P] Create `src/tools/tag/index.test.ts` — 5 registration cases (Q-49 factory returns correct shape; tool name === "tag"; description fingerprint matches baseline; schema fingerprint matches baseline; docs/tools/tag.md existence asserted by server registry test).
- [X] T031 Edit `src/server.ts` — add `createTagTool` import + tools-array entry. Place alphabetically between existing entries (likely between `read_property` and `read_heading` — verify at implementation time by inspecting the current order). Re-sort the tools array if needed.
- [X] T032 Edit `src/tools/_register.test.ts` — add a per-tool-invariants-map row for `tag`. The post-010 consolidated drift detector's `it.each` walk auto-covers the new tool once registered.
- [X] T033 Run `npm run baseline:write` to regenerate `src/tools/_register-baseline.json` — captures the new `tag` entry's description+schema fingerprints. Commit the baseline change in the same change set as T031/T032.
- [X] T034 [P] Create `docs/tools/tag.md` — published tool documentation. Include: tool description; input schema fields (`tag`, `vault?`, `total?`); output shapes (default `{count, paths}` and count-only bare integer); 6 documented inherited limitations from research.md (multi-vault basename ambiguity; Unicode case-folding NOT supported at v1; stale metadataCache; output cap inherited from cli-adapter; no pagination at v1; tag-cache filetype scope `.md`-only); 11-row failure roster from contracts/tag-input.contract.md; 8 worked examples (A-H).
- [X] T035 Edit `CHANGELOG.md` — new `## [0.5.6]` entry. Include: tool announcement (`tag` typed tool, BI-028); the two plan-stage spec amendments (case-insensitive via wrapper-side ASCII lower-fold; architecture pivot from native `tag` subcommand to eval); the six-entry failure roster (zero new top-level codes); the inherited limitations list; the third-consumer status of the shared closed-vault detector module.
- [X] T036 Edit `package.json` — version bump `0.5.5` → `0.5.6`.
- [X] T037 [P] Run quality gates: `npm run lint` (zero warnings), `npm run typecheck` (zero errors), `npm run build` (success), `vitest run` (all 53 new + existing tests pass with coverage threshold met). Capture results for the PR description's Constitution Compliance checklist.
...
- [X] T041 [P] Run `quickstart.md` Q-27 through Q-30 inspection / structural cases: ADR-010 naming check (tool name `tag`); registry baseline fingerprint roll-forward verified; Original-no-upstream attribution on every new source file; `docs/tools/tag.md` existence auto-asserted by server registry test.
- [ ] T042 SC-004 performance characterisation (≤1s @ 10 000-note vault): seed a synthetic large vault under `Sandbox/BI-028/perf/` containing ~10 000 `.md` files with a mix of tagged / untagged content, OR run the tool against a pre-existing large vault if the user has one available. Execute `tag` with a zero-match query and time end-to-end (CLI dispatch + eval JS template cache walk + handler parse). Assert wall-clock < 1 second. If the result exceeds 1s document the actual figure and decide whether to (a) accept and document as inherited limitation, (b) optimise the cache walk, or (c) escalate to the user. May be deferred to a post-implementation characterisation pass if synthetic-vault seeding is impractical in the implementation window — in that case document the deferral in CHANGELOG.md (T035) with a follow-up issue link.
- [X] T043 [P] Verify cli-adapter unknown-vault inheritance: confirm that `obsidian vault=NoSuchVault eval code=...` still emits the `Vault not found.` sentinel that the cli-adapter's 011-R5 inspection clause classifies as `CLI_REPORTED_ERROR(VAULT_NOT_FOUND, reason: "unknown")` and that the tag tool inherits this without per-handler logic. Either run a quick integration smoke against TestVault with an invalid vault name OR document the inheritance reliance explicitly in `docs/tools/tag.md` (T034) error-roster section. SC-012 coverage point.

**Checkpoint**: BI-028 complete. Registered tool surface adds `tag`; 53 new co-located vitest cases pass; baseline rolled forward; docs published; CHANGELOG entry; version bumped; quality gates clean; T0 manual verification passed against TestVault; SC-004 perf characterisation captured; SC-012 unknown-vault inheritance documented.
...
- Tag count summary: 43 tasks total. Phase 2: 5. US1: 13. US2: 2. US3: 3. US4: 5. Polish: 15. (Two tasks T042 SC-004 perf + T043 SC-012 inheritance added post-/speckit-analyze remediation.)
