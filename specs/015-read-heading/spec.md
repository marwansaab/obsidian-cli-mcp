...
- **FR-010**: The body of the named heading is defined as the contiguous bytes from the line **after** the matched heading marker up to (but not including) the **first subsequent heading marker of any depth** — be it a child-level heading (deeper depth — terminator because child subtrees are excluded), a sibling-level heading (same depth — terminator because the named heading's section ends), or a higher-level heading (shallower depth — terminator because the parent section ends). End-of-file is also a valid terminator. The returned `content` is byte-identical to the corresponding bytes of the on-disk file across this span.
...
- **FR-019**: CRLF and LF line endings in the on-disk file MUST round-trip verbatim through the read. The returned `content` MUST contain whichever line-ending bytes lived between the heading marker and the terminator on disk; bridge-side normalisation is forbidden.
...
- **Heading**: A line in a Markdown file beginning with one to six `#` characters followed by a space and the heading's text. Has a depth (1–6, where 1 is `#` and 6 is `######`) and a text payload (the rest of the line). The same heading text MAY appear multiple times in a single file at the same or different depths.
- **Heading path**: A caller-supplied string identifying a target heading by its full traversal from the document root through the heading hierarchy. Composed of two or more non-empty segments separated by the literal `::` separator. The path is matched structurally — segments are split on `::` and compared as data — not by depth marker (`#` count) or by the surrounding heading text. Paths with single segments, empty segments, or text-segments containing `::` literally are out of reach for this tool (validation rejection or out-of-scope per the documented fallback).
...
- **SC-020**: The live-CLI characterisation pass (FR-025) documents observable behaviour for all 18 cases enumerated, persisted in the feature's research artefact and surfaceable from the published documentation.
...
- The user input is exhaustive for ship-gating decisions: 15 acceptance criteria across [P1] / [P2] / [P3], six adversarial categories (CONCURRENCY, CONTENT, UNDERLYING CLI, CLIENT-CLASS, SECURITY), and an explicit out-of-scope list cover the spec surface. No clarifications session is anticipated; `/speckit-clarify` is not expected to be required before `/speckit-plan`.
- The user input contains both a parenthetical "up to the next sibling-or-higher heading, exclusive" and an explicit "Does NOT include child-heading subtrees." For headings with no children, both wordings produce the same body. For headings WITH children (where they would diverge), the spec resolves the tension by adopting the **first-subsequent-heading-marker-of-any-depth** rule (FR-010): the body terminates at whichever heading marker appears first — child, sibling, or shallower. This satisfies the "no child subtrees" wording (child markers terminate the body before child content can leak in) AND the "sibling-or-higher" wording (which holds trivially when no children exist before the next sibling/higher boundary). If the user intended the alternative interpretation — outer span with child-subtree carve-out, where prose appearing visually after a child subtree but before the next sibling is INCLUDED in the parent's body — this resolution should be challenged at the clarifications stage before plan.
