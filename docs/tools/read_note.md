...
[target-mode primitive](../../specs/004-target-mode-schema/spec.md) — read_note
adds zero tool-specific fields beyond what the primitive defines.

## Input Schema

`read_note` consumes the discriminated union below. Every field is rejected at
...
read_note propagates the adapter's classification verbatim with no rewrites.
...
The TypeScript-flavoured form: `read_note({ target_mode: "specific", vault: "MyVault", file: "Recipe" })`.
...
```json
{ "name": "read_note", "arguments": { "target_mode": "active" } }
```

The TypeScript-flavoured form: `read_note({ target_mode: "active" })`. Spawns
...
- [cli-adapter spec](../../specs/003-cli-adapter/spec.md) — the centralised
  adapter `invokeCli` that read_note routes every call through.
- [target-mode primitive spec](../../specs/004-target-mode-schema/spec.md) —
  the shared discriminated union read_note re-exports as its input schema.
