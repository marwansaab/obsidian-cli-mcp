...
[`read_note`](./read_note.md) and [`write_note`](./write_note.md): where
`read_note` retired `obsidian_exec` for reads and `write_note` retired it for
create/overwrite, `delete_note` retires it for destructive single-file
...
`delete_note` consumes the discriminated union below. Every field is rejected
...
IV. `delete_note` introduces zero new error codes — the failure surface is
...
`delete_note` propagates the adapter's classification verbatim with no
...
- [cli-adapter spec](../../specs/003-cli-adapter/spec.md) — the centralised
  `invokeCli` adapter that `delete_note` routes every call through, including
