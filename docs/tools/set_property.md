...
`set_property` consumes the discriminated union below. Every field is
...
- Write: `set_property({ ..., name: "count", value: "abc" })` (no explicit type).
...
- **Unknown vault names** surface as `CLI_REPORTED_ERROR` with the
  ...
  response-inspection clause inherited from `write_note` / `delete`).
...
Principle IV. `set_property` introduces **zero new error codes** —
...
`set_property` propagates the adapter's classification verbatim with
...
invocation, or to a `read` + body-edit + `write_note` workflow.

### Property removal — out of scope

`set_property` writes the property's value; it cannot delete a
...
- [cli-adapter spec](../../specs/003-cli-adapter/spec.md) — the
  centralised `invokeCli` adapter that `set_property` routes calls
