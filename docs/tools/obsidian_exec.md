...
Note: the centralised CLI adapter (`src/cli-adapter/cli-adapter.ts`, used by future typed tools) ALSO splits `Error: no active file` out as `ERR_NO_ACTIVE_FILE` (a recoverable user-action signal). The legacy `obsidian_exec` handler does NOT implement this priority split — it surfaces the focused-note-missing case as `CLI_REPORTED_ERROR` per its 003-spec Out-of-Scope clause. If your call hits "no active file", the failure surfaces here as `CLI_REPORTED_ERROR` with `details.message` starting `Error: no active file`.
