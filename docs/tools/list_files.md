...
list_files-specific optional fields layer on top: `folder`, `ext`,
`total`.

## Input Schema

`list_files` consumes the discriminated union below. Every field is
...
injection vector" assertion holds because `list_files` does NOT route
through `obsidian_exec` or `eval`.

## Errors

All failure surfaces flow through `UpstreamError` per Constitution
Principle IV. `list_files` introduces **zero new error codes** — the
...
`list_files` propagates the adapter's classification verbatim with no
