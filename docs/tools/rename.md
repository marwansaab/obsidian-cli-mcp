...
envelope. Direct counterpart of [`read`](./read.md),
[`write_note`](./write_note.md), and [`delete`](./delete.md):
where those retire `obsidian_exec` for read, create/overwrite, and delete
respectively, `rename` retires it for in-place rename.
...
> note are rewritten to the new name. `rename` does NOT enforce or
...
`rename` consumes the discriminated union below. Every field is
...
`rename` is **scoped to `.md` notes**. The `.md` extension-handling
...
Principle IV. `rename` introduces zero new error codes — the failure
...
`rename` propagates the adapter's classification verbatim with no
rewrites.
...
- [cli-adapter spec](../../specs/003-cli-adapter/spec.md) — the
  centralised `invokeCli` adapter that `rename` routes every call
